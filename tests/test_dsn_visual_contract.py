...
def instrument_link(**changes) -> dsn.Link:
    base = dsn.Link(
        ...
        azimuth=331.0,
        down_streams=(
            dsn.DownStream("K", 28_590_000.0, -88.0),
            dsn.DownStream("S", 146_400.0, -120.0),
        ),
        up_band="X",
    )
...
def one_stream(band: str = "X", *, rate: float = 100.0,
               dbm: float | None = None, **changes) -> dsn.Link:
    return instrument_link(
        ...
        up_kw=0.0,
        streams=1,
        down_streams=(dsn.DownStream(band, rate, dbm),),
        up_band="",
        **changes,
    )


def metric_labels(link: dsn.Link) -> list[str]:
    return [label for phase in dsn._instrument_metrics(link)
...
def test_wheel_selection_starts_and_resets_a_manual_dwell(monkeypatch):
    ...
    """
    monkeypatch.setattr(dsn, "MANUAL_DWELL_S", 120.0, raising=False)
    state = dsn.State(links=[instrument_link(craft="LRO"),
    ...
    async def scenario():
        bb = InputBar()
        listener = asyncio.create_task(dsn.listen_input(bb, state))
...
def test_auto_rotation_yields_while_manual_dwell_is_active(monkeypatch):
    ...
    monkeypatch.setattr(dsn.asyncio, "sleep", one_rotation_then_block)

    async def scenario():
        rotating = asyncio.create_task(dsn.rotate(state))
...
def test_transmit_power_never_mutates_the_top_label_pixels():
    ...
    a = dsn.render_instrument_frames(quiet)[0][0]
    b = dsn.render_instrument_frames(loud)[0][0]

    assert a.crop((0, 0, dsn.FRESH_GUTTER_X, 5)).tobytes() == \
        b.crop((0, 0, dsn.FRESH_GUTTER_X, 5)).tobytes()
...
def test_selected_instrument_scrolls_the_full_nasa_spacecraft_name(monkeypatch):
    ...
    frames, _, _ = dsn.render_instrument_frames(link, names=names)

    assert "LUNAR RECONNAISSANCE ORBITER" in seen
    assert any(frame.tobytes() != frames[0].tobytes() for frame in frames[1:])
    assert (dsn.instrument_signature(link, [], "fresh", names)
            != dsn.instrument_signature(link, [], "fresh", {}))


def test_every_band_colour_clears_the_measured_panel_step():
    colours = {band: dsn.BAND_PULSE[band] for band in ("S", "X", "K", "KA")}
...
def test_k_and_ka_are_not_the_same_visual_band():
    assert dsn.BAND_PULSE["K"] != dsn.BAND_PULSE["KA"]


def test_unknown_band_has_an_honest_distinct_colour():
    frame = dsn.render_instrument_frames(one_stream(""))[0][0]
    ...
    assert unknown not in dsn.BAND_PULSE.values()
...
def overflow_links() -> tuple[dsn.Link, dsn.Link]:
    literal = (
        dsn.DownStream("S", 100.0, -155.0),
        dsn.DownStream("S", 5_000.0, -145.0),
    )
    overflow = (
        dsn.DownStream("S", 50_000.0, -140.0),
        dsn.DownStream("S", 450_000.0, -130.0),
        dsn.DownStream("S", 5_000_000.0, -120.0),
        dsn.DownStream("S", 10_000_000.0, -110.0),
    )
    ...
    collapsed = instrument_link(
        ...
        down_streams=literal + (
            dsn.DownStream("S", sum(stream.bps for stream in overflow), -110.0),),
    )
...
def test_overflow_lane_visibly_identifies_the_real_stream_count():
    ...
    six_frames = dsn.render_instrument_frames(six)[0]
    collapsed_frames = dsn.render_instrument_frames(collapsed)[0]
...
def test_each_mode_glyph_gets_an_explanatory_text_phase(
    ...
    monkeypatch.setattr(dsn, "_text", recording_text)
    dsn.render_instrument_frames(instrument_link(**changes))
...
def test_active_power_metrics_name_rx_and_tx_compactly():
    ...
    assert max(dsn.text_width(rx[0]), dsn.text_width(tx[0])) <= 27
...
def test_normal_and_hostile_content_never_claim_the_freshness_gutter():
    hostile = instrument_link(
        ...
        up_kw=999_999.0,
        streams=1,
        down_streams=(dsn.DownStream(
            "X-BAND-WITH-A-LONG-NAME", 999_999_999_999.0, -12_345.0),),
    )
    for link in (instrument_link(), hostile):
        frames = dsn.render_instrument_frames(link, freshness="fresh")[0]
        ...
        room = dsn.INSTRUMENT_CONTENT_X1 - dsn.INSTRUMENT_X0 + 1
        for left, right in dsn._instrument_metrics(link):
            assert dsn.text_width(left) <= room
            assert dsn.text_width(right) <= room
            assert dsn.text_width(left) + 3 + dsn.text_width(right) <= room

    first = dsn.render_instrument_frames(hostile)[0][0]
    assert any(first.getpixel((x, y)) == dsn.NAME
               for x in range(dsn.INSTRUMENT_X0, dsn.INSTRUMENT_CONTENT_X1 + 1)
               for y in range(5)), "hostile dish text erased the craft identity"


def test_subsecond_light_time_is_never_rendered_as_zero():
    assert dsn.light_label(0.01) == "SUBSEC"
    assert dsn.light_label(0.99) == "SUBSEC"
...
def test_uplink_only_distance_scrolls_the_complete_status_word():
    ...
    far = dsn.light_label(link.light_s)
    assert far == "SUBSEC"
    box = (tx + dsn.text_width(far) + 3, dsn.W - 1)
    assert dsn.text_width("UPLINK") > box[1] - box[0] + 1
    ...
    offsets: list[int] = []
    cycle = dsn.text_width("UPLINK") + dsn.SCROLL_GAP_PX
    for index, frame in enumerate(frames):
        off = dsn.independent_scroll_offset(
            "UPLINK", box[1] - box[0] + 1, index, len(frames))
        offsets.append(off)
        expected = Image.new("RGB", (dsn.W, dsn.H), dsn.OFF)
        px = expected.load()
        dsn._text(px, box[0] - off, dsn.H - 5, "UPLINK", dsn.RATE,
                  clip=box)
        dsn._text(px, box[0] - off + dsn.text_width("UPLINK")
                  + dsn.SCROLL_GAP_PX, dsn.H - 5, "UPLINK", dsn.RATE,
                  clip=box)
        crop = (box[0], dsn.H - 5, box[1] + 1, dsn.H)
        assert frame.crop(crop).tobytes() == expected.crop(crop).tobytes()

        for source_x in range(dsn.text_width("UPLINK")):
    ...
    assert visible_source_columns == set(range(dsn.text_width("UPLINK")))
...
def carrier_positions(outward: bool) -> list[int]:
    positions = []
    colour = (251, 17, 193)
    for index in range(dsn.INSTRUMENT_FRAMES):
        image = Image.new("RGB", (dsn.W, dsn.H), dsn.OFF)
        dsn._carrier_marks(image.load(), 9, index / dsn.INSTRUMENT_FRAMES,
                           1, colour, outward)
        lit = [x for x in range(dsn.INSTRUMENT_X0, dsn.INSTRUMENT_X1 + 1)
...
def test_one_carrier_reaches_both_endpoints_and_has_a_clean_loop_seam(outward):
    ...
    assert min(positions) <= dsn.INSTRUMENT_X0 + 1
    assert max(positions) >= dsn.INSTRUMENT_X1 - 1
    ...
    if outward:
        assert before >= dsn.INSTRUMENT_X1 - 1
        assert after <= dsn.INSTRUMENT_X0 + 1
    else:
        assert before <= dsn.INSTRUMENT_X0 + 1
        assert after >= dsn.INSTRUMENT_X1 - 1
...
def network_links() -> list[dsn.Link]:
    ...
    goldstone = replace(
        ...
        streams=2,
        down_streams=(
            dsn.DownStream("X", 1_000.0, -140.0),
            dsn.DownStream("S", 10_000.0, -130.0),
        ),
    )
    madrid = replace(
        instrument_link(),
...
def network_frame_bytes(links: list[dsn.Link]) -> tuple[bytes, ...]:
    frames, _, _ = dsn.render_network_frames(links, freshness="fresh")
    return tuple(frame.tobytes() for frame in frames)


def test_network_view_is_a_three_site_live_72_by_16_instrument():
    frames, fps, hold = dsn.render_network_frames(
    ...
    for rows in row_bands:
        assert any(frame.getpixel((x, y)) != dsn.OFF
...
def test_empty_network_sites_show_every_pixel_of_no_link():
    ...
    frames, _, _ = dsn.render_network_page_frames([], 0, freshness="fresh")
    expected = Image.new("RGB", (dsn.W, dsn.H), dsn.OFF)
    ...
    assert expected.getpixel((39, 0)) != dsn.OFF  # K's missing outer stroke
    for frame in frames:
        for y0 in (0, 5, 10):
...
def test_network_signature_changes_exactly_with_visible_topology():
    links = network_links()
    signature = dsn.network_signature(links, freshness="fresh")
...
def test_every_paged_network_contact_gets_a_complete_friendly_name_marquee():
    ...
    for page, contact in enumerate(contacts):
        frames, fps, _ = dsn.render_network_frames(
...
def test_multiple_uplink_records_and_wind_are_visible_without_power_summing():
    ...
    one = replace(base, up_streams=(dsn.UpStream("X", 18.0, "data"),),
                  wind_kmh=8.0)
    two = replace(base, up_streams=(
        dsn.UpStream("X", 18.0, "data"),
        dsn.UpStream("S", 5.0, "ranging"),
    ), wind_kmh=11.0)

    assert tuple(dsn._instrument_metrics(two)) != tuple(dsn._instrument_metrics(one))
    assert any("2SIG" in page for page in dsn._instrument_metrics(two))
    assert ("WIND", "10KMH") in dsn._instrument_metrics(two)
    assert (dsn.render_instrument_frames(one)[0][0].tobytes()
            != dsn.render_instrument_frames(two)[0][0].tobytes())
...
def test_unknown_activity_uses_a_complete_generic_label_not_a_clipped_prefix():
    ...
    assert ("ACT", "OTHER") in dsn._instrument_metrics(unknown)
    assert ("ACT", "UNKNOWN") in dsn._instrument_metrics(missing)
    assert not any("CALIBRAT" in value
                   for page in dsn._instrument_metrics(unknown)
                   for value in page)
...
def test_multi_band_metric_splits_pages_instead_of_amputating_a_band():
    ...
    pages = dsn._instrument_metrics(mixed)
...
def test_hostile_wind_and_unknown_up_band_stay_bounded_and_honest():
    ...
    hostile = replace(
        ...
        down_streams=(),
        up_active=True,
        up_streams=(dsn.UpStream("FutureExperimentalBandName", 18.0, "data"),),
        wind_kmh=1e100,
    )

    pages = dsn._instrument_metrics(hostile)
    assert ("?", "TX") in pages
    assert ("WIND", "") in pages
    assert ("UNKNOWN", "") in pages
    frames, _, _ = dsn.render_instrument_frames(hostile)
    assert frames and all(frame.size == (dsn.W, dsn.H) for frame in frames)
...
def test_dense_instrument_metrics_extend_by_whole_rf_loops_for_readable_dwell():
    ...
    metrics = dsn._instrument_metrics(dense, "handoff")
    frames, _, _ = dsn.render_instrument_frames(
        dense, contact_state="handoff")

    assert len(frames) % dsn.INSTRUMENT_FRAMES == 0
    assert len(frames) >= len(metrics) * dsn.INSTRUMENT_METRIC_MIN_FRAMES
    page_counts = [0] * len(metrics)
    for index in range(len(frames)):
        page_counts[min(len(metrics) - 1,
                        index * len(metrics) // len(frames))] += 1
    assert min(page_counts) >= dsn.INSTRUMENT_METRIC_MIN_FRAMES
    ...
    loop_s = len(frames) / dsn.INSTRUMENT_FPS
    refresh_s = dsn.scene_refresh_s(state, dense)
...
def test_dense_metrics_do_not_slow_the_spacecraft_name_marquee():
    ...
    names = {"lro": "Lunar Reconnaissance Orbiter"}
    _, _, box, _, header_frames = dsn.instrument_header_layout(sparse, names)
    sparse_frames = dsn.render_instrument_frames(sparse, names=names)[0]
    dense_frames = dsn.render_instrument_frames(
...
def test_network_signature_uses_the_same_explicit_count_overflow_as_pixels():
    base = network_links()[0]
    ten = tuple(dsn.DownStream("X", 1000.0, -140.0) for _ in range(10))
    eleven = ten + (dsn.DownStream("X", 1000.0, -140.0),)
    ...
    assert dsn.network_signature(a, "fresh") == dsn.network_signature(b, "fresh")
    frame = dsn.render_network_page_frames(a, 0)[0][0]
    site_y = next(y for site, _, y in dsn.NETWORK_SITES
                  if site == dsn._site_name(base.complex_name))
    assert any(frame.getpixel((x, y)) == dsn.RATE
...
def test_distance_watch_names_off_air_and_handoff_instead_of_stale_live_rate(
        monkeypatch):
    link = replace(network_links()[0], range_km=dsn.C_KM_S * 120)
    labels: list[str] = []
    original = dsn._text
    ...
    dsn.render_frames(link, now, realtime_since=now.timestamp() - 1,
                      on_air=False)
