...
def _rows(peak_hour: int, probability: float, *, day: int = 11):
    base = datetime(2026, 8, day, 0, 0, tzinfo=skystrip.TZ)
    return [
        (
...
def _report(rows, *, hour: int = 10, style: str = "chicago", monkeypatch):
    ...
    now = datetime(2026, 8, 11, hour, 43, tzinfo=skystrip.TZ)
    weather = replace(skystrip.WeatherState(), temp_c=25.0, rain=True)
    return skystrip._compose_report(weather, None, now, rows)
...
def test_peak_hour_is_named_naturally(hour, expected):
    assert skystrip._peak_hour_words(hour) == expected


@pytest.mark.parametrize("hour", range(24))
def test_no_hour_is_announced_as_twelve_of_the_wrong_half(hour):
    words = skystrip._peak_hour_words(hour)
...
def test_float_forecast_probability_is_rounded(monkeypatch):
    ...
    now = datetime(2026, 8, 11, 10, 43, tzinfo=skystrip.TZ)
    text = skystrip._compose_report(
        replace(skystrip.WeatherState(), temp_c=25.0), forecast, now, None,
    )
    assert not re.search(r"\d+\.\d", text), text
