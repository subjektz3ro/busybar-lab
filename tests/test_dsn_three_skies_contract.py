...
def link(
    ...
    streams = ((dsn.DownStream(band, 2_000_000.0, -130.0),)
               if down else ())
    return dsn.Link(
        complex_name=site,
        dish=dish,
...
def render(
        links: list[dsn.Link], *, freshness: str = "fresh",
        names: dict[str, str] | None = None,
        selected_key: str | None = None,
        trails: dict[str, list[tuple[int, int]]] | None = None,
        focus: bool = False,
        ):
    return dsn.render_three_skies_frames(
        links, freshness=freshness, names=names,
        selected_key=selected_key, trails=trails, focus=focus)


def shifted_point(value: dsn.Link, centre: tuple[int, int]) -> tuple[int, int]:
    point = dsn.link_pointing_pixel(value)
    assert point is not None
    return (centre[0] + point[0] - dsn.SCOPE_CX,
            centre[1] + point[1] - dsn.SCOPE_CY)
    ...
    for frame in frames:
        for centre_x, centre_y in dsn.THREE_SKIES_SCOPE_CENTERS:
            ...
            for point in (
                ...
                assert frame.getpixel(point) != dsn.OFF
        ...
        for x in (23, 47, dsn.FRESH_GUTTER_X, dsn.FRESH_X):
            assert all(frame.getpixel((x, y)) == dsn.OFF
...
def test_site_header_numbers_contacts_not_streams_or_dishes():
    ...
    first = replace(
        first, streams=3,
        down_streams=(
            dsn.DownStream("S", 1_000.0, -140.0),
            dsn.DownStream("X", 2_000.0, -135.0),
            dsn.DownStream("K", 3_000.0, -130.0),
        ),
    )
...
def test_colliding_contacts_share_one_stable_spatial_node():
    ...
    node_colours = set(dsn.BAND_PULSE.values()) | {
        dsn.UNKNOWN_PULSE, dsn.UPLINK,
    }
    frame = frames_a[0]
    ...
    assert dsn.three_skies_signature([first, second]) == \
        dsn.three_skies_signature([second, first])
...
def test_missing_pointing_gets_a_complete_tally_and_never_a_fake_head():
    ...
    node_colours = set(dsn.BAND_PULSE.values()) | {
        dsn.UNKNOWN_PULSE, dsn.UPLINK, dsn.SCOPE_HEAD,
    }
...
def test_selection_recolours_the_exact_cell_without_moving_the_aim():
    ...
    assert plain.getpixel(expected) == dsn.BAND_PULSE["S"]
...
def test_only_selected_dish_gets_four_history_cells_plus_its_current_head():
    ...
    shift = dsn.THREE_SKIES_SCOPE_CENTERS[0][0] - dsn.SCOPE_CX

    assert {point for point in ((shift + x, 7) for x in range(1, 7))
            if frame.getpixel(point) == dsn.THREE_SKIES_TRAIL} == {
                (shift + 3, 7), (shift + 4, 7),
                (shift + 5, 7), (shift + 6, 7),
            }
    assert frame.getpixel((shift + 7, 7)) != dsn.SCOPE_TRAIL

    without_selection = render(
        [selected, other], selected_key=None, trails=trails)[0][0]
    assert all(without_selection.getpixel((x, y)) != dsn.THREE_SKIES_TRAIL
               for x in range(dsn.W) for y in range(dsn.H))
...
def test_source_freshness_never_turns_the_local_skies_into_fake_motion():
    ...
    for freshness in ("fresh", "delayed", "stale", "offline"):
        ...
        assert all(frame.getpixel((dsn.FRESH_GUTTER_X, y)) == dsn.OFF
    ...
    assert all(frame.getpixel((dsn.FRESH_X, y)) == dsn.OFF
               for frame in fresh for y in range(dsn.H))
    assert len({tuple(frame.getpixel((dsn.FRESH_X, y))
                      for y in range(dsn.H)) for frame in delayed}) > 1
    assert all(sum(frame.getpixel((dsn.FRESH_X, y)) == dsn.DELAYED
                   for y in range(dsn.H)) in (0, 3)
               for frame in delayed)
    assert all(sum(frame.getpixel((dsn.FRESH_X, y)) == dsn.STALE
                   for y in range(dsn.H)) == 3
               for frame in stale)


def test_static_triptych_uses_native_duration_folding_not_forty_full_frames():
    frames, fps, hold = render([link()])
    folded = dsn.encode_native_frames(frames, fps, hold)
    forced = dsn.anim.encode_anim(
        frames, fps=fps, durations=[hold] * len(frames))

    assert len(frames) == dsn.INSTRUMENT_FRAMES
...
def test_focus_lens_marquees_every_pixel_of_the_complete_friendly_name():
    ...
    assert len(frames) >= dsn.scroll_frame_count(label, 21)
    assert len(frames) % dsn.INSTRUMENT_FRAMES == 0
    ...
    for index, frame in enumerate(frames):
        offset = dsn.scroll_offset(label, index / len(frames), 21)
        ...
        actual = {(x, y) for x in range(box[0], box[1] + 1)
                  for y in range(6, 11)
                  if frame.getpixel((x, y)) == dsn.NAME}
...
def marquee_exposed_ink(frames, text: str, y: int,
    ...
    exposed: set[tuple[int, int]] = set()
    for index, frame in enumerate(frames):
        offset = dsn.scroll_offset(text, index / len(frames), 21)
        for copy in (0, cycle):
...
def test_focus_scrolls_complete_two_digit_index_instead_of_clipping_it():
    ...
    assert dsn.text_width(index_label) > 21
    assert marquee_exposed_ink(
        frames, index_label, 11, dsn.ANTENNA) == ink(index_label)
...
def test_focus_font_covers_every_character_in_vendored_friendly_names():
    ...
    assert source_chars <= set(dsn.FONT)
    ...
    assert marquee_exposed_ink(frames, label, 6, dsn.NAME) == ink(label)
...
def test_focus_keeps_missing_pointing_explicit_in_main_and_context_scopes():
    ...
    assert marquee_exposed_ink(frames, "1/2 ?1", 11, dsn.ANTENNA) == \
        ink("1/2 ?1")
...
def test_focus_context_status_exposes_complete_missing_and_collision_tokens(
    ...
    for index, frame in enumerate(frames):
        ...
        actual = {(x, y) for x in range(58, 62) for y in range(5)
                  if frame.getpixel((x, y)) == dsn.THREE_SKIES_LEDGER}
        assert actual == expected
        assert_complete_monochrome_text(
            frame, dsn._site_count_label("M", collision + 1), 48, 0)
        assert all(frame.getpixel((62, y)) == dsn.OFF
                   for y in range(dsn.H))
...
def test_ambient_ledger_preserves_missing_and_collision_facts_together():
    ...
    for point in ((6, 10), (6, 12), (6, 14), (7, 10), (7, 14)):
        assert frame.getpixel(point) == dsn.THREE_SKIES_LEDGER
...
def test_focus_collision_tally_is_complete_and_cannot_overwrite_the_sky(
    ...
    bracket = {(26, 11), (26, 13), (26, 15),
               (27, 11), (27, 15)}
    ledger = {(x, y) for x in range(dsn.W) for y in range(dsn.H)
              if frame.getpixel((x, y)) == dsn.THREE_SKIES_LEDGER}

    assert ledger == digit | bracket
    assert all(frame.getpixel((25, y)) == dsn.OFF for y in range(dsn.H))
    assert max(x for x, _ in ledger) < (
        dsn.THREE_SKIES_MAIN_CENTER[0] - dsn.THREE_SKIES_MAIN_R)
    projected = dsn._project_link(
        selected, *dsn.THREE_SKIES_MAIN_CENTER, dsn.THREE_SKIES_MAIN_R)
    assert projected == (28, 11), "fixture no longer exercises the old overlap"
    assert frame.getpixel(projected) == dsn.THREE_SKIES_SELECTED


def test_scope_semantic_colours_clear_the_measured_physical_panel_step():
    node_colours = tuple(dsn.BAND_PULSE.values()) + (
        dsn.UNKNOWN_PULSE, dsn.UPLINK, dsn.THREE_SKIES_SELECTED)
    for colour in node_colours:
        assert max(abs(a - b) for a, b in zip(
            dsn.THREE_SKIES_NORTH, colour)) >= PANEL_STEP
        assert max(abs(a - b) for a, b in zip(
            dsn.THREE_SKIES_TRAIL, colour)) >= PANEL_STEP
    assert max(abs(a - b) for a, b in zip(
        dsn.THREE_SKIES_TRAIL, dsn.SCOPE_RING)) >= PANEL_STEP
    for colour in node_colours + (
            dsn.SCOPE_RING, dsn.THREE_SKIES_NORTH,
            dsn.THREE_SKIES_TRAIL):
        assert max(abs(a - b) for a, b in zip(
            dsn.THREE_SKIES_LEDGER, colour)) >= PANEL_STEP


def test_rows_remains_an_explicit_runtime_fallback(monkeypatch):
    contact = link()
    state = dsn.State(links=[contact], view="network")
    ...
    assert dsn.scene_signature(state, contact, now) == dsn.network_signature(
        [contact], "fresh", state.names, page=state.network_page)
    rows, _, _ = dsn.render_network_page_frames([contact], 0)
...
def test_network_focus_is_bounded_and_never_an_ambient_auto_rotation():
    state = dsn.State(view="network")
    assert state.network_focus_until == 0.0
    assert dsn.network_focus_active(state, now=100.0) is False

    state.network_focus_until = 108.0
    assert dsn.network_focus_active(state, now=100.0) is True
    assert dsn.network_focus_active(state, now=108.0) is False


def test_only_a_rested_skies_network_pick_arms_the_focus_lens(monkeypatch):
    contact = link()
    state = dsn.State(links=[contact], view="network", picking=True)
    monkeypatch.setattr(dsn, "DSN_NETWORK_STYLE", "skies")
    ...
    assert dsn.network_focus_active(state, now=10.0) is False
...
@pytest.mark.parametrize(("age", "freshness"), [
    ((dsn.FEED_DELAYED_S + dsn.FEED_STALE_S) / 2.0, "delayed"),
    (dsn.FEED_STALE_S + 1.0, "stale"),
])
def test_picker_rest_never_arms_focus_from_an_aged_seeded_snapshot(
    ...
    monkeypatch.setattr(dsn, "DSN_NETWORK_STYLE", "skies")
    monkeypatch.setattr(dsn.time, "time", lambda: wall_now)
    assert dsn.feed_freshness(state) == freshness

    dsn.commit_picker_selection(state, now=123.0)
...
def test_reconcile_clears_exact_link_focus_without_stranding_ambient_selection(
    ...
    events = dsn.reconcile_links(state, incoming, now=200.0)
...
def runtime_state(*contacts: dsn.Link) -> dsn.State:
    now = time.time()
    state = dsn.State(links=list(contacts), view="network")
...
def test_push_scene_uses_three_skies_and_never_touches_rows(monkeypatch):
    ...
    bb = RuntimeBar()
    calls = []
    real_render = dsn.render_three_skies_frames
    ...
    monkeypatch.setattr(dsn, "DSN_NETWORK_STYLE", "skies")
    monkeypatch.setattr(dsn, "render_three_skies_frames", recording_skies)
    monkeypatch.setattr(dsn, "render_network_page_frames",
                        row_renderer_must_not_run)
    monkeypatch.setattr(dsn, "start_network_page_warm",
                        row_prewarm_must_not_run)
    monkeypatch.setattr(dsn.anim, "encode_anim",
                        lambda *args, **kwargs: b"three-skies")

    accepted = asyncio.run(dsn.push_scene(
...
def test_accepted_focus_starts_exactly_one_native_loop_after_acceptance(
    ...
    expected_loop = dsn.three_skies_loop_s(
    ...
    assert dsn.network_focus_active(
        state, now=state.network_focus_until - 0.001)
    assert not dsn.network_focus_active(
...
def test_focus_snapshot_does_not_restart_when_new_source_geometry_arrives(
    ...
    monkeypatch.setattr(dsn, "DSN_NETWORK_STYLE", "skies")
    dsn.commit_picker_selection(state, now=10.0)
    before = dsn.scene_signature(state, contact)
    ...
    after = dsn.scene_signature(state, state.links[0])
...
def test_event_cards_wait_until_the_deliberate_focus_lens_finishes(
    ...
    async def scenario():
        ...
        return await dsn.show_next_event(bb, state)
...
def test_expired_focus_signature_is_the_ambient_triptych_signature(
    ...
    async def scenario():
        ...
        focus_signature = dsn.scene_signature(state, state.current())
        ...
        assert not dsn.network_focus_active(state, now=loop_now)
        expired_signature = dsn.scene_signature(state, state.current())

        state.network_focus_until = 0.0
        state.network_focus_key = None
        ambient_signature = dsn.scene_signature(state, state.current())
...
def test_skies_never_advances_or_prewarms_the_row_page_machine(monkeypatch):
    ...
    assert dsn.advance_network_page_if_due(state, due=True) is False
    assert state.network_page == 2
    assert state.network_page_pending is False
    assert dsn.start_network_page_warm(object(), state) is None
...
def test_refused_changing_scenes_keep_the_uploaded_cache_bounded(monkeypatch):
    ...
    async def scenario():
        for index in range(dsn.SCENE_CACHE_MAX + 3):
...
def handoff_event(
    ...
    events = dsn.visual_events([old], [new], now=time.time())
...
def release_event_card(state: dsn.State) -> None:
...
def test_handoff_echo_phases_exact_cells_then_complete_embedded_label():
    ...
    frames, fps, hold = dsn.render_handoff_echo_frames(target)

    assert len(frames) == dsn.EVENT_FRAMES == 20
    assert fps == dsn.EVENT_FPS == 5
    assert hold == 1
    ...
    assert all(frame.getpixel(new_point) == dsn.OFF for frame in old_frames)
    assert all(frame.getpixel(old_point) == dsn.OFF for frame in new_frames)
    ...
    for point, phase in ((old_point, old_frames), (new_point, new_frames)):
        ...
        assert all(
            frame.getpixel(neighbour) == dsn.OFF
            for frame in phase
    ...
    label = dsn.event_label(target)
    full_ink = ink(label)
    label_width = max(x for x, _y in full_ink) + 1
    x0 = (dsn.W - label_width) // 2
...
def test_handoff_echo_signature_includes_geometry_and_embedded_label():
    ...
    signature = dsn.handoff_echo_signature(target)
    assert signature is not None
    assert dsn.handoff_echo_signature(renamed) is not None
    assert dsn.handoff_echo_signature(renamed) != signature
    assert dsn.handoff_echo_signature(moved) != signature
...
def test_skies_handoff_uploads_one_shot_echo_with_no_native_text_overlay(
    ...
    assert animation.timeout == dsn.EVENT_TIMEOUT_S == 4
    assert event_text(payload) is None
    assert state.active_event_label == dsn.event_label(target)
...
def test_handoff_echo_ineligible_label_uses_complete_native_text_only(
    ...
    monkeypatch.setattr(dsn, "encode_native_frames", must_not_encode)
    ...
    assert native.text == dsn.event_label(target)
...
def test_skies_handoff_with_invalid_pointing_is_text_only_and_never_uploads(
    ...
    assert bb.uploads == []
    assert event_animation(bb.draws[-1]) is None
    assert event_text(bb.draws[-1]).text == dsn.event_label(target)
    assert state.active_event_embedded_label is False
    assert state.scene_cache == {}
    assert state.event_queue == []
...
def test_picker_retires_embedded_handoff_without_creating_event_text(
    ...
    async def scenario():
        ...
        assert state.active_event_embedded_label is True
        state.picking = True
        await dsn.draw_picker(bb, state)
...
def test_handoff_echo_scene_cache_is_bounded_across_changing_geometry(
    ...
    signatures = {dsn.handoff_echo_signature(target) for target in targets}
    assert len(signatures) > dsn.SCENE_CACHE_MAX, \
    ...
    assert len(state.scene_cache) <= dsn.SCENE_CACHE_MAX
    assert len(bb.removed) >= len(signatures) - dsn.SCENE_CACHE_MAX
...
def test_picker_wins_if_it_arrives_during_a_handoff_echo_upload(monkeypatch):
    ...
    assert labels == [dsn.picker_label(state)]
    assert state.event_queue == [target]
    assert len(bb.uploads) == 1
