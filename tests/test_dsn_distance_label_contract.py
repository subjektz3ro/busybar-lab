...
These tests deliberately rasterise the finite labels without calling dsn_render_text._text.
...
BOTTOM_Y = dsn_limits.H - 5
DIVIDER = (40, 55, 75)


def link_at(light_s: float | None = 1 / 3, *, bps: float | None = 0.0,
            receiving: bool = False, up: bool = True) -> dsn_source.Link:
    streams = ((dsn_source.DownStream("X", bps, -140.0),) if receiving else ())
    return dsn_source.Link(
        complex_name="Canberra", dish="DSS43", craft="MMS2",
        elevation=21.0, band="X", down_bps=bps, up_active=up,
        range_km=(dsn_source.C_KM_S * light_s if light_s is not None else None),
...
def source_ink(text: str) -> set[tuple[int, int]]:
    """The complete unclipped source ink, independent of dsn_render_text._text."""
    ...
    for ch in text.upper():
        ...
        for y, row in enumerate(glyph):
            for dx, bit in enumerate(row):
                if bit == "1":
                    result.add((x + dx, y))
        x += len(glyph[0]) + dsn_render_text.GLYPH_GAP
    return result
...
def paint_text(image: Image.Image, text: str, x: int,
    ...
    for source_x, source_y in source_ink(text):
        target_x = x + source_x
        if not (0 <= target_x < dsn_limits.W):
            continue
...
def expected_bottom(far: str, fast: str, frame_index: int,
    ...
    """Compose the full bottom row from semantic layout rules, not dsn_render_text._text."""
    ...
    room = dsn_limits.W - 1 - tx

    assert far_width <= room
    if not live:
        paint_text(expected, far, tx, dsn_render_palette.DIST)

    if far_width + 3 + fast_width <= room:
        fast_x = dsn_limits.W - 1 - fast_width
        fast_box = (fast_x, dsn_limits.W - 1)
        paint_text(expected, fast, fast_x, dsn_render_palette.RATE)
    else:
        fast_x = tx + far_width + 3
        fast_box = (fast_x, dsn_limits.W - 1)
        box_width = fast_box[1] - fast_box[0] + 1
        cycle = fast_width + dsn_render_text.SCROLL_GAP_PX
        offset = dsn_render_text.independent_scroll_offset(
            fast, box_width, frame_index, frame_count)
        paint_text(expected, fast, fast_box[0] - offset, dsn_render_palette.RATE, fast_box)
        paint_text(expected, fast, fast_box[0] - offset + cycle,
                   dsn_render_palette.RATE, fast_box)

    gap0, gap1 = tx + far_width, fast_x
    divider_x = -1
    if gap1 - gap0 >= 3:
        divider_x = (gap0 + gap1) // 2
        for y in range(BOTTOM_Y, dsn_limits.H):
...
def test_every_distance_status_and_rate_preserves_the_complete_bottom_row(
        link: dsn_source.Link, render_args: dict, far: str, fast: str) -> None:
    frames, _, _ = dsn_render_distance.render_frames(link, NOW, **render_args)
    live = render_args.get("realtime_since") is not None
    row_box = (dsn_render_palette.GLOBE_CX + dsn_render_palette.GLOBE_R + 3, dsn_limits.W - 1)
    row_crop = inclusive_crop(row_box, BOTTOM_Y, dsn_limits.H)
    ...
    for index, frame in enumerate(frames):
        ...
        far_crops.add(frame.crop(inclusive_crop(far_box, BOTTOM_Y, dsn_limits.H)).tobytes())
        if divider_x >= 0:
            divider_crops.add(frame.crop(
                inclusive_crop((divider_x, divider_x), BOTTOM_Y, dsn_limits.H)
            ).tobytes())
    ...
    if offsets:
        ...
        cycle = ink_width(fast) + dsn_render_text.SCROLL_GAP_PX
        ...
        crop = inclusive_crop(first_box, BOTTOM_Y, dsn_limits.H)
