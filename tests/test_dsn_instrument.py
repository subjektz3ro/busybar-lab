...
def _row_positions(frame, y: int, colour: tuple[int, int, int]) -> list[int]:
    ...
    return [x for x in range(dsn.INSTRUMENT_X0 + 5, dsn.INSTRUMENT_X1 - 3)
            if px[x, y] == colour]


def _carrier_region(frame) -> tuple[tuple[tuple[int, int, int], ...], ...]:
    px = frame.load()
    return tuple(tuple(px[x, y] for x in range(dsn.INSTRUMENT_X0,
                                                dsn.INSTRUMENT_X1 + 1))
                 for y in range(6, 11))
...
def test_feed_retains_source_time_pointing_and_each_real_stream():
    assert dsn.feed_timestamp_ms(FEED) == 1_786_145_549_000
...
@pytest.mark.parametrize(
    ("azimuth", "expected"),
    [(0.0, (dsn.SCOPE_CX, dsn.SCOPE_CY - dsn.SCOPE_R)),
     (90.0, (dsn.SCOPE_CX + dsn.SCOPE_R, dsn.SCOPE_CY)),
     (180.0, (dsn.SCOPE_CX, dsn.SCOPE_CY + dsn.SCOPE_R)),
     (270.0, (dsn.SCOPE_CX - dsn.SCOPE_R, dsn.SCOPE_CY))],
)
def test_pointing_pixel_puts_horizon_on_the_true_cardinal(azimuth, expected):
    assert dsn.pointing_pixel(azimuth, 0.0) == expected


def test_pointing_pixel_puts_zenith_at_the_centre_and_clamps_elevation():
    centre = (dsn.SCOPE_CX, dsn.SCOPE_CY)
    for azimuth in (0.0, 73.0, 180.0, 359.0):
        assert dsn.pointing_pixel(azimuth, 90.0) == centre
        assert dsn.pointing_pixel(azimuth, 1000.0) == centre
    assert dsn.pointing_pixel(360.0, -10.0) == dsn.pointing_pixel(0.0, 0.0)
...
def test_rate_buckets_change_only_at_declared_thresholds(value, expected):
    assert dsn.rate_bucket(value) == expected
...
def test_receive_power_buckets_change_only_at_declared_thresholds(value, expected):
    assert dsn.receive_power_bucket(value) == expected
...
def test_transmit_power_buckets_change_only_at_declared_thresholds(value, expected):
    assert dsn.transmit_power_bucket(value) == expected
...
def test_instrument_signature_ignores_jitter_that_changes_no_physical_led():
    link = instrument_link(
        ...
        down_dbm=-88.10,
        down_streams=(
            dsn.DownStream("K", 28_590_010.0, -88.10),
            dsn.DownStream("S", 146_410.0, -120.10),
        ),
    )
    jittered = replace(
        link,
        ...
        down_dbm=-88.20,
        down_streams=(
            dsn.DownStream("K", 28_590_020.0, -88.20),
            dsn.DownStream("S", 146_420.0, -120.20),
        ),
    )
    assert dsn.pointing_pixel(link.azimuth, link.elevation) == \
        dsn.pointing_pixel(jittered.azimuth, jittered.elevation)
    assert dsn.instrument_signature(link, [], "fresh") == \
        dsn.instrument_signature(jittered, [], "fresh")
...
def test_instrument_signature_changes_for_visible_or_semantic_changes():
    base = instrument_link(
        ...
        up_kw=0.9,
        streams=1,
        down_streams=(dsn.DownStream("S", 999.0, -151.0),),
    )
    signature = dsn.instrument_signature(base, [], "fresh")
    variants = [
        replace(base, azimuth=90.0),
        replace(base, down_bps=1_000.0,
                down_streams=(dsn.DownStream("S", 1_000.0, -151.0),)),
        replace(base, down_dbm=-149.0,
                down_streams=(dsn.DownStream("S", 999.0, -149.0),)),
        replace(base, up_kw=1.0),
        replace(base, down_streams=(dsn.DownStream("X", 999.0, -151.0),)),
        replace(base, arrayed=True),
    ]
    for changed in variants:
        assert dsn.instrument_signature(changed, [], "fresh") != signature
    assert dsn.instrument_signature(base, [(1, 1)], "fresh") != signature
    assert dsn.instrument_signature(base, [], "delayed") != signature


def test_instrument_animation_has_one_fixed_device_native_clock():
    frames, fps, hold = dsn.render_instrument_frames(instrument_link())
    assert len(frames) == dsn.INSTRUMENT_FRAMES == 40
    assert fps == dsn.INSTRUMENT_FPS == 5
    assert hold == 1
...
def test_only_fresh_carriers_move():
    link = instrument_link()
    fresh, _, _ = dsn.render_instrument_frames(link, freshness="fresh")
    delayed, _, _ = dsn.render_instrument_frames(link, freshness="delayed")
    stale, _, _ = dsn.render_instrument_frames(link, freshness="stale")
...
def test_fresh_animation_leaves_the_rail_to_the_native_live_lease():
    frames, _, _ = dsn.render_instrument_frames(instrument_link(), freshness="fresh")
...
def test_uplink_and_downlink_move_in_their_literal_directions():
    link = instrument_link(
        ...
        down_dbm=-140.0,
        streams=1,
        down_streams=(dsn.DownStream("S", 100.0, -140.0),),
    )
    frames, _, _ = dsn.render_instrument_frames(link, freshness="fresh")

    up0 = _row_positions(frames[0], 6, dsn.UPLINK)
    up1 = _row_positions(frames[1], 6, dsn.UPLINK)
    down0 = _row_positions(frames[0], 9, dsn.BAND_PULSE["S"])
    down1 = _row_positions(frames[1], 9, dsn.BAND_PULSE["S"])
...
def test_each_downsignal_gets_its_own_band_coloured_lane():
    frames, _, _ = dsn.render_instrument_frames(instrument_link())
    assert any(dsn.BAND_PULSE["K"] in [frame.getpixel((x, 8))
                                      for x in range(dsn.INSTRUMENT_X0,
                                                     dsn.INSTRUMENT_X1 + 1)]
               for frame in frames)
    assert any(dsn.BAND_PULSE["S"] in [frame.getpixel((x, 10))
                                      for x in range(dsn.INSTRUMENT_X0,
                                                     dsn.INSTRUMENT_X1 + 1)]
               for frame in frames)
...
def test_zero_rate_stream_is_a_silent_tether_not_a_fake_carrier():
    link = instrument_link(
        ...
        down_dbm=-140.0,
        streams=1,
        down_streams=(dsn.DownStream("X", 0.0, -140.0),),
    )
    frames, _, _ = dsn.render_instrument_frames(link, freshness="fresh")
    for frame in frames:
        interior = [frame.getpixel((x, 9))
                    for x in range(dsn.INSTRUMENT_X0 + 5,
                                   dsn.INSTRUMENT_X1 + 1)]
        assert dsn.BAND_PULSE["X"] not in interior
...
def test_missing_live_fields_stay_unknown_instead_of_becoming_strong_x_band():
    ...
    assert dsn.link_pointing_pixel(link) is None
    assert link.band == ""
    assert link.down_streams[0].band == ""
    assert link.down_dbm is None
    assert dsn.receive_power_bucket(link.down_streams[0].dbm) == 0
    assert dsn._instrument_metrics(link)[0] == ("?", "0BPS")

    frame = dsn.render_instrument_frames(link)[0][0]
    assert all(frame.getpixel((x, y)) != dsn.SCOPE_HEAD
               for x in range(dsn.W) for y in range(dsn.H))
    # Unknown power gets no invented weak flare; the endpoint is only tether.
    assert frame.getpixel((dsn.INSTRUMENT_X0, 9)) == dsn.INSTRUMENT_TETHER


def test_nonfinite_numeric_fields_are_missing_not_maximal():
    assert dsn._f("NaN", 7.0) == 7.0
    assert dsn._f("inf", 7.0) == 7.0
    assert dsn.receive_power_bucket(float("nan")) == 0


def test_instrument_contrast_clears_the_measured_physical_panel_floor():
    assert max(dsn.INSTRUMENT_TETHER) >= 77
    assert max(dsn.SCOPE_TRAIL) - max(dsn.SCOPE_RING) >= 77
    weak_flare = (175, 105, 35)
    assert max(weak_flare) - max(dsn.INSTRUMENT_TETHER) >= 77
...
def test_array_mspa_and_ddor_glyphs_have_distinct_composable_geometry():
    ...
    expected = {
        dsn.ANTENNA: _glyph_points(61, array_rows),
        dsn.NAME: _glyph_points(64, mspa_rows),
        dsn.DDOR_MARK: _glyph_points(67, ddor_rows),
    }
