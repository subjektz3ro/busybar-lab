...
async def test_restoring_over_an_expired_scene_leaves_the_panel_lit():
    ...
    await skystrip.restore_current_view(bb, state)
...
async def test_acknowledging_a_live_warning_never_ends_in_a_dark_panel():
    ...
    handled = await skystrip.acknowledge_alert(bb, state, "test")
...
async def test_the_stale_notice_is_retired_by_the_scene_that_replaces_it():
    ...
    retired = skystrip._retired_stale_notice_elements(state)
    ...
    assert not skystrip._retired_stale_notice_elements(skystrip.SkyState()), (
...
async def test_the_stale_notice_never_covers_an_alert_or_time_machine():
    ...
    for field, value in (("visual_alert", _tornado_warning()), ("scrub_slot", 12)):
        ...
        assert not await skystrip.keep_stale_notice(bb, state)
...
async def test_a_booting_app_does_not_announce_an_outage_it_has_not_had():
    ...
    state.stale_since -= skystrip.STALE_NOTICE_GRACE_S
    assert await skystrip.keep_stale_notice(bb, state)
...
async def test_the_stale_notice_refreshes_on_its_own_cadence():
    ...
    state = skystrip.SkyState()
    state.stale_since = (
        asyncio.get_running_loop().time() - skystrip.STALE_NOTICE_GRACE_S
    )
    bb = RecordingBar()
    ...
    state.stale_notice_at -= skystrip.STALE_REDRAW_S
    assert await skystrip.keep_stale_notice(bb, state)
...
async def test_the_notice_outlives_nothing_and_its_lease_beats_its_cadence():
    ...
    assert skystrip.STALE_REDRAW_S < skystrip.STALE_ELEMENT_TIMEOUT_S
    assert skystrip.STALE_ELEMENT_TIMEOUT_S <= skystrip.ELEMENT_TIMEOUT_S
...
async def test_acknowledgement_demands_a_fresh_scene_instead_of_the_cached_one():
    ...
    await skystrip.acknowledge_alert(bb, state, "START")
...
def test_the_notice_text_fits_the_condensed_budget():
    ...
    assert len(skystrip.STALE_WEATHER_TEXT) <= 12
