...
def test_every_publishable_mission_blurb_is_stable_or_has_a_sourced_lease():
    assert set(dsn_missions.MISSIONS) == set(dsn_missions.MISSION_REVIEWS)
    assert dsn_missions.STABLE_MISSION_BLURBS <= set(dsn_missions.MISSIONS)
    for code, review in dsn_missions.MISSION_REVIEWS.items():
        current = dsn_missions.mission_blurb(code, dsn_missions.MISSION_REVIEWED_ON)
        if review.stable:
            assert review.reviewed_on is not None, code
            assert review.review_by is None, code
            assert current == dsn_missions.MISSIONS[code]
            assert dsn_missions.mission_blurb(code, date(2100, 1, 1)) == current
        elif current:
            ...
            expired = dsn_missions.mission_blurb(code, date(2100, 1, 1))
...
def test_unsourced_status_is_silent_but_sourced_status_retains_a_lease():
    unreviewed = dsn_missions.MISSION_REVIEWS["tgo"]
    assert not unreviewed.stable
    assert not unreviewed.source_url
    assert dsn_missions.mission_blurb("tgo", dsn_missions.MISSION_REVIEWED_ON) == ""

    voyager = dsn_missions.MISSION_REVIEWS["vgr2"]
    assert not voyager.stable
    assert voyager.source_url == "https://science.nasa.gov/mission/voyager/"
    assert voyager.reviewed_on == dsn_missions.MISSION_REVIEWED_ON
    assert voyager.review_by == dsn_missions.MISSION_REVIEW_BY
    assert "interstellar space" in dsn_missions.mission_blurb(
        "vgr2", dsn_missions.MISSION_REVIEWED_ON)
    assert dsn_missions.mission_blurb("vgr2", date(2100, 1, 1)) == ""

    mro = dsn_missions.MISSION_REVIEWS["mro"]
    assert mro.source_url == "https://science.nasa.gov/mars/mars-relay-network/"
    assert "relays data home" in dsn_missions.mission_blurb(
        "mro", dsn_missions.MISSION_REVIEWED_ON)
    assert dsn_missions.mission_blurb("mro", date(2100, 1, 1)) == ""
...
def test_confirmed_stale_aliases_are_phase_truthful(
    ...
    words = dsn_missions.MISSIONS[code].lower()
    assert must_include in words
    assert must_exclude not in words
    assert dsn_missions.MISSION_REVIEWS[code].source_url.startswith("https://")


def test_generated_fleet_aliases_do_not_generalise_status():
    reviewed_codes = set()
    for code in ("go10", "go11", "go12", "go14", "go16", "go17"):
        assert "watching" not in dsn_missions.MISSIONS[code].lower()
        reviewed_codes.add(code)
    for code in ("go13", "go15"):
        assert "retir" not in dsn_missions.MISSIONS[code].lower()
        reviewed_codes.add(code)
    for code in ("no15", "no16", "no17", "no18"):
        assert "retired in august 2025" in dsn_missions.MISSIONS[code].lower()
        reviewed_codes.add(code)
    for code in ("clu1", "clu2", "clu3", "clu4"):
        assert "ended the cluster mission" in dsn_missions.MISSIONS[code].lower()
        reviewed_codes.add(code)
    for code in ("thb", "thc"):
        assert "moved into lunar orbit" in dsn_missions.MISSIONS[code].lower()
        assert "renamed artemis" in dsn_missions.MISSIONS[code].lower()
        reviewed_codes.add(code)

    assert "retirement" in dsn_missions.MISSIONS["tdr1"].lower()
    assert "retirement" in dsn_missions.MISSIONS["tdr4"].lower()
    assert "retir" not in dsn_missions.MISSIONS["tdr3"].lower()
    assert "completed in 2022" in dsn_missions.MISSIONS["em-1"].lower()
    assert "completed in april 2026" in dsn_missions.MISSIONS["em-2"].lower()
    assert "low-earth-orbit" in dsn_missions.MISSIONS["em-3"].lower()
    assert "flight designation" in dsn_missions.MISSIONS["em-4"].lower()
    reviewed_codes.update(("tdr1", "tdr3", "tdr4", "em-1", "em-2",
                           "em-3", "em-4"))
    assert all(dsn_missions.MISSION_REVIEWS[code].source_url.startswith("https://")
               for code in reviewed_codes)


def test_expired_artemis_status_falls_back_to_phase_neutral_copy():
    words = dsn_missions.mission_blurb("em3", date(2100, 1, 1)).lower()
...
def test_k_and_ka_services_are_not_conflated():
    k_words = dsn_audio_words.band_words("K").lower()
    ka_words = dsn_audio_words.band_words("Ka").lower()
...
def test_receive_records_are_spoken_per_record_not_as_summed_throughput():
    records = (
        dsn_source.DownStream("X", 1_000.0, -140.0, "data"),
        dsn_source.DownStream("X", 2_000.0, -141.0, "data"),
    )
...
def test_many_receive_records_have_bounded_exact_nonaggregating_speech():
    records = tuple(
        dsn_source.DownStream("X", float(index + 1) * 1_000.0, -140.0, "data")
        for index in range(dsn_source.FEED_SIGNAL_RECORDS_PER_DISH_MAX))
    words = dsn_audio_words.receive_records_words(records, len(records))
...
def test_unknown_source_band_tokens_are_not_read_as_supported_services():
    ...
    records = (
        dsn_source.DownStream("X", 1_000.0, -140.0, "data"),
        dsn_source.DownStream(hostile, 2_000.0, -141.0, "data"),
    )
    words = dsn_audio_words.spoken(
...
def test_missing_range_draws_stationary_nonmetric_carriers():
    ...
    unknown, _, _ = dsn_render_distance.render_frames(
        _link(craft="LUCY", range_km=None), when, {"lucy": "Lucy"})
    known, _, _ = dsn_render_distance.render_frames(
        _link(craft="LUCY"), when, {"lucy": "Lucy"})

    def rf_rows(frame):
        return tuple(
            frame.getpixel((x, y))
            for y in (dsn_render_palette.UP_Y, dsn_render_timing.DOWN_Y)
...
def test_controlled_attitude_spacecraft_do_not_receive_invented_glints():
    assert "lucy" not in dsn_render_craft.CRAFT_GLINT
    assert "clipper" not in dsn_render_craft.CRAFT_GLINT
