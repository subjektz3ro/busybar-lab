...
def _link(*, craft: str = "VGR2", dish: str = "DSS14",
    ...
    return dsn_source.Link(
        ...
        down_streams=(dsn_source.DownStream("X", 160.0, -140.0),), streams=1,
    )
...
def test_timestamp_parser_and_freshness_reject_unbounded_epochs():
    ...
    assert dsn_source.feed_timestamp_ms(huge) is None

    state = dsn_model.State(
        feed_timestamp_ms=int("9" * 4000), feed_advanced_at=1_800_000_000.0)
    assert dsn_telemetry.feed_freshness(state, now=1_800_000_000.0) == "stale"


def test_fixture_counter_cannot_drive_a_fresh_production_scene():
    now = 1_800_000_000.0
    assert dsn_source.source_timestamp_valid(1, now) is False
    state = dsn_model.State(feed_timestamp_ms=1, feed_advanced_at=now)
    assert dsn_telemetry.feed_freshness(state, now=now) == "stale"
...
def test_site_codes_are_canonical_without_friendly_names():
    assert dsn_source.parse_feed(_feed(1_000, station="gdscc"))[0].complex_name == \
        "Goldstone"
    assert dsn_source.parse_feed(_feed(1_000, station="mdscc"))[0].complex_name == \
        "Madrid"
    assert dsn_source.parse_feed(_feed(1_000, station="cdscc"))[0].complex_name == \
        "Canberra"


def test_config_is_complete_finite_and_range_checked():
    fixture = Path("tests/fixtures/dsn_config.xml").read_bytes()
    names, dishes, sites = dsn_source.parse_config(fixture)
    ...
    for invalid in (b'longitude="NaN"', b'longitude="Infinity"',
        ...
        with pytest.raises(ValueError, match="longitude"):
            dsn_source.parse_config(poisoned)
    ...
    with pytest.raises(ValueError, match="missing DSN sites"):
        dsn_source.parse_config(partial)
...
def test_invalid_config_does_not_partially_replace_last_known_good(monkeypatch):
    ...
    assert asyncio.run(dsn_feed.fetch_names(state)) is False
    assert (state.names, state.dish_types, state.site_lons) == before


def test_source_identity_limits_reject_instead_of_truncate():
    long_code = "X" * (dsn_source.SOURCE_CODE_MAX + 1)
    with pytest.raises(dsn_source.SourceValidationError, match="oversized.*identity"):
        dsn_source.parse_feed(_feed(1_000, craft=long_code))
    with pytest.raises(ValueError, match="bounded document size"):
        dsn_source.parse_feed(b" " * (dsn_source.FEED_XML_MAX_BYTES + 1))

    fixture = Path("tests/fixtures/dsn_config.xml").read_bytes()
    extra = (f'<spacecraft name="long" friendlyName="'
             f'{"X" * (dsn_source.SOURCE_NAME_MAX + 1)}"/>').encode()
    bounded = fixture.replace(b"</spacecraftMap>", extra + b"</spacecraftMap>")
    with pytest.raises(dsn_source.SourceValidationError,
                       match="oversized config spacecraft friendly name"):
        dsn_source.parse_config(bounded)

    enormous_id = "9" * 4000
    hostile_id = _feed(1_000).replace(
        b'spacecraftID="-32"', f'spacecraftID="{enormous_id}"'.encode()).replace(
        b'id="32"', f'id="{enormous_id}"'.encode())
    assert dsn_source.parse_feed(hostile_id)[0].naif is None
    ...
    assert len(future_dish) <= dsn_source.SOURCE_DISH_CODE_MAX
    assert dsn_source.parse_feed(_feed(1_000, dish=future_dish))[0].dish == future_dish
...
def test_extreme_source_ranges_remain_unknown_and_cannot_expand_narration():
    ...
    parsed = dsn_source.parse_feed(hostile)[0]

    assert parsed.range_km is None
    assert parsed.light_s is None
    assert dsn_audio_words.distance_words(float("inf")) == ""
    assert dsn_audio_words.distance_words(1e300) == ""
    with pytest.raises(ValueError, match="invalid Horizons observer range"):
        dsn_ranges.horizons_au("$$SOE\n2026-Aug-08 00:00 1e300\n$$EOE")
...
def test_rejected_identity_snapshot_cannot_manufacture_a_link_loss(monkeypatch):
    ...
    hostile = _feed(
        int(now * 1000), craft="X" * (dsn_source.SOURCE_CODE_MAX + 1))
...
def test_maximum_accepted_collection_has_a_complete_bounded_roster_plan():
    feed = _dense_feed(
        dsn_source.FEED_DISHES_PER_SITE_MAX, dsn_source.FEED_LINKS_PER_DISH_MAX)
    assert len(feed) < dsn_source.FEED_XML_MAX_BYTES
    links = dsn_source.parse_feed(feed)
    assert len(links) == (
        dsn_source.FEED_DISHES_PER_SITE_MAX * dsn_source.FEED_LINKS_PER_DISH_MAX)

    groups = dsn_render_network_dishes.group_links_by_dish(links, "Goldstone")
    width = dsn_render_network_dishes._dish_roster_width(groups)
    travel_capacity = (
        dsn_limits.SCROLL_SPEED_PX_S * dsn_limits.MAX_ANIMATION_FRAMES
        / dsn_limits.INSTRUMENT_FPS)
    assert width + dsn_render_text.SCROLL_GAP_PX <= travel_capacity
    assert dsn_render_network_dishes.dish_network_frame_count(links) <= dsn_limits.MAX_ANIMATION_FRAMES

    longest = links[0]
    names = {longest.craft.lower(): "W" * dsn_source.SOURCE_NAME_MAX}
    moving = dsn_render_distance.distance_header_layout(longest, names)[3]
    assert dsn_render_text.text_width(moving) + dsn_render_text.SCROLL_GAP_PX <= travel_capacity
    assert dsn_render_distance.distance_frame_count(longest, names) <= dsn_limits.MAX_ANIMATION_FRAMES


@pytest.mark.parametrize(("dishes", "links_per_dish", "message"), [
    (dsn_source.FEED_DISHES_PER_SITE_MAX + 1, 1, "too many dishes"),
    (1, dsn_source.FEED_LINKS_PER_DISH_MAX + 1, "too many active links"),
])
def test_over_budget_valid_feed_is_rejected_as_one_snapshot(
        dishes, links_per_dish, message):
    with pytest.raises(dsn_source.SourceValidationError, match=message):
        dsn_source.parse_feed(_dense_feed(dishes, links_per_dish))


def test_config_collection_limit_is_fail_closed():
    fixture = Path("tests/fixtures/dsn_config.xml").read_bytes()
    count = dsn_source.CONFIG_SPACECRAFT_MAX - 168 + 1
    extras = b"".join(
        f'<spacecraft name="z{index}" friendlyName="Z {index}"/>'.encode()
        for index in range(count))
    oversized = fixture.replace(
        b"</spacecraftMap>", extras + b"</spacecraftMap>")
    assert len(oversized) < dsn_source.CONFIG_XML_MAX_BYTES
    with pytest.raises(dsn_source.SourceValidationError, match="too many spacecraft"):
        dsn_source.parse_config(oversized)


def test_unknown_dish_size_remains_unknown():
    assert dsn_formatting.dish_metres("DSS14", {}) is None
    assert dsn_formatting.dish_metres("DSS999", {"DSS14": "70M"}) is None
    assert dsn_formatting.dish_metres("DSS14", {"DSS14": "70M"}) == "70"


def test_range_cache_is_versioned_validated_and_range_sensitive(
        tmp_path, monkeypatch):
    cache = tmp_path / "ranges.json"
    monkeypatch.setattr(dsn_settings, "RANGE_CACHE", cache)
    now = 1_800_000_000.0
    monkeypatch.setattr(time, "time", lambda: now)

    cache.write_text(json.dumps({
        "version": dsn_limits.RANGE_CACHE_VERSION,
        "ranges": {
            "-151": [70_000.0, now - dsn_limits.RANGE_NEAR_EARTH_TTL_S - 1],
            "-32": [21_000_000_000.0, now - 3600],
            "-61": [820_000_000.0, now + 1],
        },
    }))
    state = dsn_model.State()
    dsn_ranges.load_ranges(state)
    assert state.ranges == {-32: (21_000_000_000.0, now - 3600)}

    dsn_ranges.save_ranges(state)
    saved = json.loads(cache.read_text())
    assert saved["version"] == dsn_limits.RANGE_CACHE_VERSION
    assert set(saved["ranges"]) == {"-32"}
    assert dsn_ranges.range_ttl_s(70_000.0) == dsn_limits.RANGE_NEAR_EARTH_TTL_S
    assert dsn_ranges.range_ttl_s(21_000_000_000.0) == dsn_limits.RANGE_TTL_S
...
def test_malformed_valid_json_cache_is_ignored_atomically(
    ...
    state = dsn_model.State(ranges=original.copy())
...
def test_one_horizons_query_fills_every_link_with_the_same_naif(monkeypatch):
    ...
    state = dsn_model.State(links=[first, second], feed_seeded=True)
    calls = []
    ...
    monkeypatch.setattr(httpx, "AsyncClient", Client)
    monkeypatch.setattr(asyncio, "sleep", stop_after_query)
...
def test_signal_and_link_duplicates_are_canonical_not_aggregated():
    ...
    link = dsn_source.parse_feed(duplicate_signal)[0]
    ...
    two_rates = single.replace(
        ...
        b'''        <downSignal active="true" spacecraft="VGR2"
                    ...
                    power="-141"/>
        <target''', 1)
    link = dsn_source.parse_feed(two_rates)[0]
    ...
    duplicate_block = single.replace(
        b"      </dish>", b"      </dish>\n      <dish" + block
        + b"      </dish>", 1)
    assert len(dsn_source.parse_feed(duplicate_block)) == 1

    contradiction = duplicate_block.replace(b'dataRate="160"',
                                            b'dataRate="161"', 1)
    with pytest.raises(dsn_source.SourceValidationError,
                       match="contradictory duplicate DSN link"):
        dsn_source.parse_feed(contradiction)


def test_band_and_native_text_normalization_cover_source_variants():
    for variant in ("K", " k ", "K BAND", "K-band", "k_band"):
        assert dsn_source.band_key(variant) == "K"

    state = dsn_model.State(links=[_link(craft="ZÜRICH")])
    assert dsn_device_display.picker_label(state) == "ZURICH 1/1"
    handoff = dsn_render_labels.event_label({
        "event": "handoff", "craft": "ZÜRICH",
        "from_dish": "DSS14", "dish": "DSS43",
    })
    assert handoff == "ZURICH 14>43"

    payloads = (dsn_device_display._picker_payload("MÜNCHEN"),
                dsn_device_display._event_payload("MÜNCHEN"),
                dsn_device_display._status_payload("MÜNCHEN"))
