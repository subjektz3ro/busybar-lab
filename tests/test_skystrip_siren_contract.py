...
def test_generated_siren_is_deterministic_bar_pcm_without_clipping():
    first = skystrip.siren_pcm()
    second = skystrip.siren_pcm()

    assert first is second
    assert len(first) == skystrip.SIREN_SECONDS * 44_100 * 2
...
async def test_siren_upload_is_content_addressed_and_reused_per_process():
    ...
    blob = skystrip.siren_pcm()
    expected = f"siren_{hashlib.sha256(blob).hexdigest()[:16]}.snd"
    state = skystrip.SkyState()
    bar = Bar()

    assert await skystrip.ensure_siren_asset(bar, state) == expected
    assert bar.uploads == [(skystrip.APP_NAME, expected, blob)]
    assert await skystrip.ensure_siren_asset(bar, state) == expected
...
async def test_wrong_sized_resident_siren_gets_new_immutable_repair_path(
        monkeypatch):
    assert skystrip.SIREN_RETIRE_GRACE_S >= skystrip.SIREN_SECONDS
    monkeypatch.setattr(skystrip, "SIREN_RETIRE_GRACE_S", 0.01)
    blob = skystrip.siren_pcm()
    ...
    assert await skystrip.ensure_siren_asset(bar, state) == repaired
    ...
    await skystrip.retire_siren_assets(bar, state)
...
async def test_play_404_is_noncommitting_and_repairs_without_global_stop(
    ...
    base = f"siren_{digest}.snd"
    repaired = f"siren_{digest}_r01.snd"
    state = skystrip.SkyState(siren_file=base)
    ...
    with pytest.raises(exceptions.BusyBarAPIError):
        await skystrip._play_audio(bar, state, base, "alert", lambda: True)
    assert state.audio_owner is None
    assert state.audio_stop_pending is False

    skystrip.mark_siren_unplayable(state, base)
    assert await skystrip.ensure_siren_asset(bar, state) == repaired
...
async def test_daemon_retries_transient_siren_provision_failure(monkeypatch):
    ...
    bar.fail_uploads = 1
    state = skystrip.SkyState()
    monkeypatch.setattr(skystrip, "SIREN_PROVISION_RETRY_S", 0.01)

    # Production attempts once during startup, which also generates/caches the
    # PCM before the lifetime maintainer starts.
    assert await skystrip.ensure_siren_asset(bar, state) is None
    assert await skystrip.ensure_siren_asset(bar, state) is not None
    assert [op for op, _name in bar.operations].count("upload") == 2

    retry_state = skystrip.SkyState()
    attempts = 0
    ...
    monkeypatch.setattr(skystrip, "ensure_siren_asset", provision)
    maintainer = asyncio.create_task(
        skystrip.maintain_siren_asset(StorageBar(), retry_state))
    try:
