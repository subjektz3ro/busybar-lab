...
def test_blank_uses_the_dedicated_repo_cache_directory(tmp_path):
    path, warning = dsn_config.resolve_cache_dir("", tmp_path)
    assert path == tmp_path / "cache" / "dsn"
    assert warning == ""
    assert dsn_config.resolve_cache_dir(None, tmp_path)[0] == tmp_path / "cache" / "dsn"


def test_a_path_inside_the_checkout_is_honoured(tmp_path):
    inside = tmp_path / "config" / "caches"
    path, warning = dsn_config.resolve_cache_dir(str(inside), tmp_path)
...
def test_an_existing_outside_directory_is_honoured(tmp_path):
    ...
    elsewhere.mkdir()
    path, warning = dsn_config.resolve_cache_dir(str(elsewhere), tmp_path)
...
def test_a_nonexistent_outside_path_is_refused_not_created(tmp_path):
    ...
    path, warning = dsn_config.resolve_cache_dir(str(target), tmp_path)
...
def test_the_fallback_is_reported_not_silent(tmp_path):
    _, warning = dsn_config.resolve_cache_dir("/nonexistent/somewhere", tmp_path)
    assert warning and "/nonexistent/somewhere" in warning


def test_a_malformed_value_falls_back(tmp_path):
    path, warning = dsn_config.resolve_cache_dir("\x00bad", tmp_path)
    assert path == tmp_path / "cache" / "dsn"
    assert warning


def test_malformed_managed_root_cannot_crash_module_startup(tmp_path):
    path, warning = dsn_config.resolve_managed_cache_root("\x00bad", tmp_path)
...
def test_managed_child_accepts_only_the_installer_managed_cache_root(tmp_path):
    ...
    path, warning = dsn_config.resolve_cache_dir(
        str(selected),
        tmp_path,
