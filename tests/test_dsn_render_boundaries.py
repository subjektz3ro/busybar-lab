...
def link(*, site: str = "Goldstone", dish: str = "DSS14",
         ...
         up: bool = True) -> dsn.Link:
    streams = ((dsn.DownStream("X", 1_000.0, -140.0),) if down else ())
    return dsn.Link(
        ...
        up_streams=((dsn.UpStream("X", 18.0),) if up else ()),
    )
...
def test_distance_signature_includes_visible_receive_presence():
    ...
    unknown_receive = replace(
        uplink, streams=1, down_streams=(dsn.DownStream("", None, None),))
    state = dsn.State(links=[uplink], view="distance", names={})

    assert dsn.scene_signature(state, uplink, NOW) != \
        dsn.scene_signature(state, unknown_receive, NOW)
    a = dsn.render_frames(uplink, NOW)[0]
    b = dsn.render_frames(unknown_receive, NOW)[0]
...
def test_distance_signature_keeps_distinct_spacing_inside_one_text_label():
    first = link()
    first = replace(first, range_km=dsn.C_KM_S)
    second = replace(first, range_km=dsn.C_KM_S * 1.2)
    state = dsn.State(links=[first], view="distance")

    assert dsn.light_label(first.light_s) == dsn.light_label(second.light_s)
    assert dsn.represented_packet_spacing(first.light_s, False) != \
        dsn.represented_packet_spacing(second.light_s, False)
    assert dsn.scene_signature(state, first, NOW) != \
        dsn.scene_signature(state, second, NOW)
    first_frames = dsn.render_frames(first, NOW)[0]
    second_frames = dsn.render_frames(second, NOW)[0]
...
def test_future_dish_identity_is_complete_in_distance_and_instrument():
    ...
    dish, sep, box, header, combined, frame_count = \
        dsn.distance_header_layout(contact, {})
    assert dish == "1234567890"
    assert sep is None and combined
    assert header == "1234567890/FUTURE"
    distance = dsn.render_frames(contact, NOW)[0]
    assert len(distance) == frame_count <= dsn.MAX_ANIMATION_FRAMES
    # Both semantic colour segments must enter the final composed box.
    assert any(frame.getpixel((x, y)) == dsn.DISH_NO
               for frame in distance for x in range(box[0], box[1] + 1)
               for y in range(5))
    assert any(frame.getpixel((x, y)) == dsn.NAME
               for frame in distance for x in range(box[0], box[1] + 1)
               for y in range(5))

    inst_dish, inst_sep, inst_box, inst_header, inst_frames = \
        dsn.instrument_header_layout(contact, {})
    assert inst_dish == "" and inst_sep is None
    assert inst_header == header
    instrument = dsn.render_instrument_frames(contact)[0]
    assert len(instrument) >= inst_frames
    assert exposed_ink(
        instrument[:inst_frames], inst_header, inst_box, 0, dsn.NAME
...
def test_future_dish_identity_is_complete_in_rows_and_three_skies():
    ...
    rows = dsn.render_network_page_frames([contact], 0)[0]
    assert exposed_ink(rows, suffix, (6, 15), 0, dsn.DISH_NO) == \
        source_ink(suffix)

    skies = dsn.render_three_skies_frames(
        [contact], selected_key=contact.key)[0]
    assert exposed_ink(skies, suffix, (0, 8), 10, dsn.DISH_NO) == \
        source_ink(suffix)

    identity = f"G{suffix}"
    focus = dsn.render_three_skies_frames(
        [contact], selected_key=contact.key, focus=True)[0]
    assert exposed_ink(focus, identity, (0, 20), 0, dsn.DISH_NO) == \
        source_ink(identity)
...
def test_three_skies_validates_same_dish_aim_before_pixel_quantization():
    ...
    assert dsn._project_link(first, 63, 7, 6) == \
        dsn._project_link(second, 63, 7, 6)

    groups, missing = dsn._group_scope_links(
...
def test_three_skies_uses_visible_overflow_tokens_not_false_nines():
    ...
    frame = dsn.render_three_skies_frames(contacts)[0][0]

    assert dsn._site_count_label("G", 9) == "G9"
    assert dsn._site_count_label("G", 12) == "G>"
    expected = {(x, y) for x, y in source_ink("G>")}
    assert all(frame.getpixel(point) == dsn.ANTENNA for point in expected)
...
def test_dish_roster_uses_a_bounded_independent_clock_not_row_lcm():
    ...
    frames = dsn.render_dish_network_frames(contacts)[0]

    assert len(frames) == dsn.dish_network_frame_count(contacts)
    assert dsn.INSTRUMENT_FRAMES <= len(frames) <= dsn.MAX_ANIMATION_FRAMES
    assert len(frames) < 1_200
    for site, _, _ in dsn.NETWORK_SITES:
        groups = dsn.group_links_by_dish(contacts, site)
        width = dsn._dish_roster_width(groups)
        if width <= 59:
            continue
        cycle = width + dsn.SCROLL_GAP_PX
        duration = len(frames) / dsn.INSTRUMENT_FPS
        turns = max(1, math.floor(
            dsn.SCROLL_SPEED_PX_S * duration / cycle))
        assert turns * cycle / duration <= dsn.SCROLL_SPEED_PX_S
...
def test_distance_long_names_extend_the_native_loop_without_speeding_rf():
    ...
    frames = dsn.render_frames(contact, NOW, names)[0]
    _, _, box, label, _, planned = dsn.distance_header_layout(contact, names)

    assert len(frames) == planned > dsn.ANIM_FRAMES
    assert len(frames) <= dsn.MAX_ANIMATION_FRAMES
    cycle = dsn.text_width(label) + dsn.SCROLL_GAP_PX
    duration = len(frames) / dsn.ANIM_FPS
    turns = max(1, math.floor(dsn.SCROLL_SPEED_PX_S * duration / cycle))
    assert turns * cycle / duration <= dsn.SCROLL_SPEED_PX_S
    # RF still repeats its exact eight-second source clock.
    rf_box = (dsn.TRACK0, dsn.UP_Y, dsn.TRACK1 + 1, dsn.DOWN_Y + 1)
    assert frames[0].crop(rf_box).tobytes() == \
        frames[dsn.ANIM_FRAMES].crop(rf_box).tobytes()
...
def test_distance_refresh_is_aligned_to_the_extended_native_loop():
    ...
    state = dsn.State(
        ...
        names={"future": "X RAY MULTI MIRROR OBSERVATORY WITH LONG NAME"})
    loop_s = dsn.distance_loop_s(contact, state.names)
    refresh_s = dsn.scene_refresh_s(state, contact)
...
def test_equal_distance_clock_keys_use_one_exact_render_timestamp():
    contact = link()
    state = dsn.State(links=[contact], view="distance")
    cadence = dsn.scene_refresh_s(state, contact)
    ...
    first_bucket, first_anchor = dsn.distance_render_clock(
        state, contact, first)
    second_bucket, second_anchor = dsn.distance_render_clock(
        state, contact, second)

    assert first_bucket == second_bucket
    assert first_anchor == second_anchor
    assert dsn.scene_signature(state, contact, first) == \
        dsn.scene_signature(state, contact, second)
    first_frames = dsn.render_frames(contact, first_anchor)[0]
    second_frames = dsn.render_frames(contact, second_anchor)[0]
    assert [frame.tobytes() for frame in first_frames] == \
        [frame.tobytes() for frame in second_frames]


def test_meaning_bearing_distance_tether_clears_physical_panel_step():
    assert max(dsn.TETHER) >= 77
    assert max(dsn.UP_TETHER) >= 77
...
def test_instrument_overflow_lane_does_not_sum_receive_rates(monkeypatch):
    contact = replace(
        link(), streams=4,
        down_streams=tuple(
            dsn.DownStream("X", 1_000_000.0, -140.0)
            for _ in range(4)),
        down_bps=None)
    counts: list[int] = []
    original = dsn._carrier_marks
    ...
    monkeypatch.setattr(dsn, "_carrier_marks", record)
    dsn.render_instrument_frames(contact)
    assert 0 in counts
...
def test_distance_signature_uses_the_same_canonical_site_as_the_renderer():
    ...
    state = dsn.State(links=[contact], view="distance")
    unknown = dsn.scene_signature(state, contact, NOW)
    unknown_frames = dsn.render_frames(contact, NOW, site_lons={})[0]

    state.site_lons = {"Goldstone": -116.8895382}
    known = dsn.scene_signature(state, contact, NOW)
    known_frames = dsn.render_frames(
        contact, NOW, site_lons=state.site_lons)[0]

    assert dsn._site_name("gdscc") == "Goldstone"
...
def test_dish_focus_bounds_cross_product_of_long_names_and_many_contacts():
    contacts = [link(craft=f"C{index}")
                for index in range(dsn.FEED_LINKS_PER_DISH_MAX)]
    names = {contact.craft.lower(): "W" * dsn.SOURCE_NAME_MAX
             for contact in contacts}

    frames = dsn.render_dish_focus_frames(
        contacts, names=names, selected_key=contacts[0].key)[0]
    summary = f"+{len(contacts) - 1} TARGETS"

    assert len(frames) <= dsn.MAX_ANIMATION_FRAMES
    assert exposed_ink(
        frames, summary, dsn.DISH_FOCUS_CRAFT_BOX, 11,
        dsn.DISH_NETWORK_COUNT) == source_ink(summary)
