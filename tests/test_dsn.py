...
def test_uplink_only_link_still_gets_an_id():
    ...
    mro = next(l for l in dsn.parse_feed(feed) if l.craft == "MRO")
...
def test_crossing_time_keeps_the_real_ratios():
    ...
    juno = dsn.crossing_seconds(3138.0)                  # 52 light-minutes
    voyager = dsn.crossing_seconds(71382.0)              # 19.8 light-hours
    ...
    assert dsn.crossing_seconds(None) == dsn.LOOP_S
...
def test_the_inner_solar_system_does_not_collapse_onto_one_number():
    ...
    moon = dsn.crossing_seconds(1.3)
    lagrange = dsn.crossing_seconds(5.0)
    mars_near = dsn.crossing_seconds(190.0)
    mars_far = dsn.crossing_seconds(1300.0)
    ...
    assert moon >= dsn.CROSS_MIN_S
    assert moon < 1.0 and mars_near < 2.0
    # and the law is continuous where the two branches meet
    knee = dsn.CROSS_KNEE_S
    assert dsn.crossing_seconds(knee - 0.5) == pytest.approx(
        dsn.crossing_seconds(knee), rel=0.02), "discontinuity at the knee"
...
def test_loop_duration_is_fixed_so_scrolling_is_decoupled():
    ...
    rates = set()
    for link in links():
        _, fps, hold = dsn.render_frames(link, now)
        rates.add((fps, hold))
...
def test_distance_shows_up_as_packets_in_flight():
    ...
    track = 44
    near = dsn.packet_spacing(dsn.crossing_seconds(960), track)      # Mars
    far = dsn.packet_spacing(dsn.crossing_seconds(71382), track)     # Voyager
    ...
    for light_s in (960, 3138, 71382):
        cross = dsn.crossing_seconds(light_s)
        speed = dsn.packet_spacing(cross, track) / dsn.LOOP_S
        assert speed == pytest.approx(track / cross, rel=0.01)


def test_light_label_reads_as_time():
    assert dsn.light_label(5) == "5SEC"
    assert dsn.light_label(3138) == "52M"
    assert dsn.light_label(71382) == "19.8H"
    assert dsn.light_label(None) == "?"


def test_frames_are_panel_sized_and_seamless_count():
    link = next(l for l in links() if l.craft == "VGR2")
    frames, fps, hold = dsn.render_frames(link, datetime.now(timezone.utc))
    assert len(frames) == dsn.ANIM_FRAMES
    assert all(f.size == (dsn.W, dsn.H) for f in frames)
    assert 1 <= fps <= 12
...
def test_the_message_travels_toward_earth():
    ...
    link = next(l for l in links() if l.craft == "VGR2")
    frames, _, _ = dsn.render_frames(link, datetime.now(timezone.utc))
    ...
    def pulse_x(frame):
        ...
        warm = [(frame.getpixel((x, row))[0] - frame.getpixel((x, row))[2], x)
                for x in range(dsn.GLOBE_CX + dsn.GLOBE_R + 1, dsn.W)]
        best, x = max(warm)
...
def test_every_glyph_we_can_draw_exists_in_the_font():
    ...
    samples = [dsn.craft_label(l.craft, names) for l in links()]
    samples += [f"{dsn.light_label(x)} {dsn.rate_label(y)}"
                for x in (None, 5, 3138, 71382) for y in (0, 160, 18090, 2e6)]
    samples += [l.dish.replace("DSS", "") for l in links()]
    for text in samples:
        for ch in text.upper():
            assert ch in dsn.FONT, f"{ch!r} (in {text!r}) has no glyph"
...
def test_full_name_is_never_truncated():
    ...
    assert dsn.craft_label("VGR2", names) == "VOYAGER 2"
    assert dsn.craft_label("ACE", names) == "ADVANCED COMPOSITION EXPLORER"
    assert dsn.craft_label("XYZ", {}) == "XYZ"         # no map, no problem


def test_short_names_do_not_scroll():
    box = 55
    for phase in (0.0, 0.25, 0.5, 0.99):
        assert dsn.scroll_offset("JUNO", phase, box) == 0
...
def test_long_names_scroll_one_whole_cycle_per_loop():
    ...
    box = 55
    cycle = dsn.text_width(label) + dsn.SCROLL_GAP_PX
    assert dsn.scroll_offset(label, 0.0, box) == 0
    assert dsn.scroll_offset(label, 0.999, box) == pytest.approx(cycle, abs=2)
    offs = [dsn.scroll_offset(label, i / 40, box) for i in range(40)]
...
def test_scrolling_label_stays_inside_its_box():
    ...
    px = img.load()
    dsn._text(px, 4, 0, "ADVANCED COMPOSITION EXPLORER", (255, 255, 255),
              clip=(20, 50))
    lit = [x for x in range(dsn.W) for y in range(dsn.H) if px[x, y] != (0, 0, 0)]
...
def test_globe_is_round_without_cardinal_spikes():
    ...
    from PIL import Image
    img = Image.new("RGB", (dsn.W, dsn.H), (0, 0, 0))
    dsn._globe(img.load(), 0.0, 0.0, 0.0)
    px = img.load()
    top = [x for x in range(dsn.W) if px[x, dsn.GLOBE_CY - dsn.GLOBE_R] != (0, 0, 0)]
    assert len(top) != 1, "single pixel spike at the top of the globe"
    widest = max(sum(1 for x in range(dsn.W) if px[x, y] != (0, 0, 0))
                 for y in range(dsn.H))
    tallest = max(sum(1 for y in range(dsn.H) if px[x, y] != (0, 0, 0))
                  for x in range(dsn.W))
...
def test_spoken_report_is_plain_ascii():
    ...
    for link in links():
        text = dsn.spoken(link)
        assert text.isascii(), text
        assert "None" not in text


def test_sweep_regex_matches_our_generations_only():
    assert dsn.GENERATION_FILES.match("dsn_12345_7.anim")
    assert not dsn.GENERATION_FILES.match("dsn_12345_7.snd"), \
        "scene cleanup must never claim speech assets"
    assert not dsn.GENERATION_FILES.match("sky_12345_1.anim")
    assert not dsn.GENERATION_FILES.match("notes.txt")
...
def test_encoder_delta_reads_through_the_input_nesting():
    assert dsn.encoder_delta(wheel(3)) == 3
    assert dsn.encoder_delta(wheel(-2)) == -2
    assert dsn.encoder_delta({"input": {}}) == 0
    assert dsn.encoder_delta({}) == 0
    # the bug: the event at the top level must NOT be mistaken for a real one
    assert dsn.encoder_delta({"encoder_event": {"delta": 4}}) == 0


def test_empty_button_event_is_an_ok_press():
    """proto3 omits zero values, so {} really is OK+PRESS."""
    assert dsn.is_ok_press({"input": {"button_event": {}}})
    assert dsn.is_ok_press({"input": {"button_event": {"button": "OK", "action": "PRESS"}}})
    assert not dsn.is_ok_press({"input": {}})
    assert not dsn.is_ok_press({"input": {"button_event": {"action": "RELEASE"}}})
    assert not dsn.is_ok_press({"input": {"button_event": {"button": "START"}}})


def test_start_press_is_distinct_from_ok():
    assert dsn.is_start_press({"input": {"button_event": {"button": "START"}}})
    assert dsn.is_start_press({"input": {"button_event": {"button": 2}}})
    assert not dsn.is_start_press({"input": {"button_event": {}}})   # that's OK
    assert not dsn.is_start_press({"input": {"button_event":
...
def test_one_felt_click_moves_one_signal():
    ...
    assert dsn.DETENT_COUNTS == 1
    state = dsn.State()
    state.links = [object()] * 4
    moved = []
    for raw in (1, 1, 1, -1):
        state.enc_accum += raw
        while abs(state.enc_accum) >= dsn.DETENT_COUNTS:
            step = 1 if state.enc_accum > 0 else -1
            state.enc_accum -= dsn.DETENT_COUNTS * step
            moved.append(step)
...
def _voice_link(**kw):
    base = dict(complex_name="Canberra", dish="DSS43", craft="VGR2",
                ...
                range_km=2.1e10)
    base.update(kw)
    return dsn.Link(**base)


def test_narration_says_antenna_mission_and_light_time():
    text = dsn.spoken(_voice_link(), {"vgr2": "Voyager 2"})
...
def test_acronym_names_are_not_title_cased():
    ...
    assert "SOHO" in dsn.spoken(_voice_link(craft="SOHO"), {"soho": "SOHO"})
    assert "Soho" not in dsn.spoken(_voice_link(craft="SOHO"), {"soho": "SOHO"})


def test_a_parenthetical_is_stripped_from_the_spoken_name():
    name = dsn.spoken_name("DSCO", {"dsco": "Deep Space Climate Observatory (DSCOVR)"})
    assert name == "Deep Space Climate Observatory"


def test_units_are_singular_when_there_is_one_of_them():
    assert dsn.light_words(1.28) == "1 second"     # the Moon
    assert dsn.light_words(60.0) == "60 seconds"   # under 90s stays in seconds
    assert dsn.light_words(3600.0) == "1 hour"
    assert dsn.light_words(3660.0) == "1 hour and 1 minute"
    assert dsn.light_words(2.1e10 / dsn.C_KM_S).startswith("19 hours and")


def test_uplink_only_does_not_say_transmitting_twice():
    text = dsn.spoken(_voice_link(down_bps=0.0), {"vgr2": "Voyager 2"})
...
def test_the_same_line_always_gets_the_same_filename():
    ...
    assert dsn.speech_name(dsn.spoken(a, {})) == dsn.speech_name(dsn.spoken(b, {}))
    other = _voice_link(craft="JNO", dish="DSS25", complex_name="Goldstone")
    assert dsn.speech_name(dsn.spoken(other, {})) != dsn.speech_name(dsn.spoken(a, {}))
    assert dsn.VOICE_FILES.match(dsn.speech_name("anything"))
...
def test_a_jittering_bitrate_does_not_rebake_the_line():
    ...
    a = dsn.spoken(_voice_link(down_bps=2_048_000.0), {})
    b = dsn.spoken(_voice_link(down_bps=2_050_113.0), {})
    assert dsn.speech_name(a) == dsn.speech_name(b)
...
def test_speaking_holds_the_rotation_on_the_narrated_link():
    ...
    async def scenario():
        state.speech[dsn.speech_name(dsn.spoken(link, state.names))] = 0.0
        task = asyncio.create_task(dsn.speak(bb, state, link))
...
def test_speaking_does_not_steal_a_lock_the_user_set():
    ...
    state.speech[dsn.speech_name(dsn.spoken(link, state.names))] = 0.0
    asyncio.run(dsn.speak(_SpeakBar(), state, link))
...
def test_a_cache_hit_uploads_nothing():
    ...
    link = _voice_link()
    text = dsn.spoken(link, {})
    state.speech[dsn.speech_name(text)] = 4.0
    bb = _SpeakBar()
    name, seconds = asyncio.run(dsn.ensure_speech(bb, state, text))
    assert bb.uploads == [], "re-uploaded a line the device already has"
    assert (name, seconds) == (dsn.speech_name(text), 4.0)


def test_the_voice_cache_stays_bounded():
    state = dsn.State()
    bb = _SpeakBar()
    for i in range(dsn.SPEECH_CACHE_MAX + 4):
        state.speech[f"voice_{i:012x}.snd"] = 1.0
    asyncio.run(dsn.trim_speech_cache(bb, state))
    assert len(state.speech) == dsn.SPEECH_CACHE_MAX
...
def test_failed_voice_asset_removal_keeps_the_deterministic_mapping():
    state = dsn.State()
    for i in range(dsn.SPEECH_CACHE_MAX + 1):
    ...
    asyncio.run(dsn.trim_speech_cache(BrokenStorage(), state))
    assert len(state.speech) == dsn.SPEECH_CACHE_MAX + 1
...
def test_a_line_in_constant_use_is_never_evicted(monkeypatch):
    ...
    monkeypatch.setattr(dsn, "synth_snd", fake_synth)
    monkeypatch.setattr(dsn, "isolate_tts_process", lambda: False)
    state, bb = dsn.State(), _SpeakBar()
    favourite = "Canberra is listening to Voyager 2."

    async def scenario():
        await dsn.ensure_speech(bb, state, favourite)
        for i in range(dsn.SPEECH_CACHE_MAX * 2):
            await dsn.ensure_speech(bb, state, f"Madrid is listening at {i} kilobits.")
            await dsn.ensure_speech(bb, state, favourite)   # ...and used again

    asyncio.run(scenario())
    assert synthesised.count(favourite) == 1, "re-baked a line that was in constant use"
    assert dsn.speech_name(favourite) in state.speech
    assert len(state.speech) == dsn.SPEECH_CACHE_MAX
...
def test_one_rotation_warms_the_cache_for_good(monkeypatch):
    ...
    async def lap():
        for text in rotation:
            await dsn.ensure_speech(bb, state, text)
    ...
    assert len(synthesised) == len(rotation), (
        ...
        f"{len(synthesised) - len(rotation)} evicted from a {dsn.SPEECH_CACHE_MAX}-slot cache")
...
def test_non_linux_synthesis_runs_on_a_daemon_thread(monkeypatch):
    ...
    monkeypatch.setattr(dsn, "synth_snd", fake_synth)
    monkeypatch.setattr(dsn, "isolate_tts_process", lambda: False)
    pcm = asyncio.run(dsn.synth_off_loop("hello"))
...
def test_a_failing_synth_propagates_rather_than_hanging(monkeypatch):
    ...
    monkeypatch.setattr(dsn, "synth_snd", boom)
    monkeypatch.setattr(dsn, "isolate_tts_process", lambda: False)
    with pytest.raises(RuntimeError):
        asyncio.run(dsn.synth_off_loop("hello"))

    # ensure_speech turns that into a None, not a crashed task
    state = dsn.State()
    assert asyncio.run(dsn.ensure_speech(_SpeakBar(), state, "hello")) is None
    assert state.speech == {}
...
def test_linux_synthesis_uses_a_disposable_worker(monkeypatch):
    ...
    monkeypatch.setattr(dsn, "isolate_tts_process", lambda: True)
    monkeypatch.setattr(dsn, "synth_snd", forbidden)
    monkeypatch.setattr(dsn.asyncio, "create_subprocess_exec", create)

    pcm = asyncio.run(dsn.synth_off_loop("hello from Earth"))
...
def test_cancelling_an_isolated_bake_terminates_its_process(monkeypatch):
    ...
    async def scenario():
        ...
        task = asyncio.create_task(dsn.synth_off_loop("cancel me"))
...
def test_the_voice_is_part_of_the_cache_key():
    ...
    assert dsn.speech_name(text, "af_nova") != dsn.speech_name(text, "am_michael")
    assert dsn.speech_name(text, "af_nova") == dsn.speech_name(text, "af_nova")
    assert dsn.speech_name(text) == dsn.speech_name(text, dsn.VOICE)


def test_the_filename_says_which_voice_recorded_it():
    text = "Canberra is listening to Voyager 2."
    assert dsn.speech_name(text, "af_nova").startswith("v2_afnova_")
    assert dsn.speech_name(text, "am_michael").startswith("v2_ammichael_")
    for name in (dsn.speech_name(text, "af_nova"), dsn.speech_name(text, "Karen")):
        assert dsn.VOICE_FILES.match(name), name
...
def test_long_voice_names_are_distinct_and_fit_the_firmware_filename_buffer():
    ...
    names = [dsn.speech_name(text, voice) for voice in voices]

    assert names[0] != names[1]
    assert all(dsn.VOICE_FILES.fullmatch(name) for name in names)
    assert all(
...
def test_startup_reclaims_lines_recorded_by_another_voice():
    ...
    mine = dsn.speech_name("a line", dsn.VOICE)
    theirs = dsn.speech_name("a line", "some_other_voice")
...
def test_dry_run_renders_every_link_without_a_device():
    ...
    out = dsn.describe_links(links, {"vgr2": "Voyager 2",
...
def test_each_label_sits_at_the_end_it_describes():
    ...
    frames, _, _ = dsn.render_frames(link, datetime(2026, 8, 6, 12, tzinfo=timezone.utc),
                                     {"vgr2": "VOYAGER 2"})
    px = frames[0].load()

    def lit_columns(y0, y1):
        return {x for x in range(dsn.W) for y in range(y0, y1)
                if px[x, y] != (0, 0, 0)}

    top = lit_columns(0, 5)
    globe_edge = dsn.GLOBE_CX + dsn.GLOBE_R
...
def test_dish_size_comes_from_nasa_not_a_hardcoded_list():
    ...
    assert dsn.dish_metres("DSS99", live) == "70"
    assert dsn.dish_metres("DSS43", live) == "34"      # live type overrides it
    assert dsn.dish_metres("DSS43", {}) is None
    assert dsn.dish_metres("DSS25", None) is None
    text = dsn.spoken(_voice_link(dish="DSS43"), {}, {"DSS43": "70M"})
...
def test_the_picker_names_the_signal_and_its_place_in_the_list():
    ...
    state.cursor = 1
    assert dsn.picker_label(state) == "SOHO 2/3"
    state.cursor = 2
    assert dsn.picker_label(state) == "VGR2 3/3"
    state.cursor = 5                                  # wraps, never indexes off
    assert dsn.picker_label(state) == "VGR2 3/3"
    assert dsn.picker_label(dsn.State()) == "NO SIGNAL"
...
def test_the_picker_fits_the_device_font_budget():
    ...
    for i in range(len(state.links)):
        state.cursor = i
        assert len(dsn.picker_label(state)) <= 12, dsn.picker_label(state)
...
def test_the_picker_keeps_identical_geometry_on_every_redraw():
    ...
    a = dsn._picker_payload("JNO 1/5", timeout=3)
    b = dsn._picker_payload("SOHO 2/5", timeout=1)
...
def test_native_readout_scrolls_complete_long_labels_instead_of_slicing():
    ...
    payload = dsn._picker_payload(label, timeout=2)
...
def test_progress_runs_from_the_craft_to_earth():
    ...
    assert dsn.realtime_progress(light, lock, lock) == pytest.approx(0.0)
    assert dsn.realtime_progress(light, lock + 250, lock) == pytest.approx(0.25)
    assert dsn.realtime_progress(light, lock + 999, lock) == pytest.approx(0.999)
    # and it STOPS at arrival — the watch ends there, it does not restart
    assert dsn.realtime_progress(light, lock + 1001, lock) == 1.0
    assert dsn.realtime_progress(0, 123.0, 0.0) == 0.0
    assert dsn.realtime_progress(None, 123.0, 0.0) == 0.0


def test_progress_advances_at_the_true_speed_of_light():
    light = 2.1e10 / dsn.C_KM_S
    track = dsn.TRACK1 - dsn.TRACK0
    per_pixel = light / track                          # ~27 min on Voyager
    lock = 1_780_000_000.0
    ts = lock + light * 0.25                           # mid-flight, clear of
                                                       # the arrival wrap
    moved = (dsn.realtime_progress(light, ts + per_pixel, lock)
             - dsn.realtime_progress(light, ts, lock)) * track
...
def test_redraw_cadence_tracks_how_fast_the_chain_actually_moves():
    ...
    voyager = 2.1e10 / dsn.C_KM_S
    assert 15 < dsn.realtime_redraw_s(mars) < 30, "Mars should visibly creep"
    assert dsn.realtime_redraw_s(voyager) == 300.0, "clamped, not once a day"
    # a near craft is animated by the device itself; no fast host redraws
    assert dsn.realtime_redraw_s(1.3) == float(dsn.REDRAW_S)
    assert dsn.realtime_redraw_s(None) == float(dsn.REDRAW_S)
...
def test_locking_switches_the_chain_off_the_browsing_speed():
    ...
    browse, _, _ = dsn.render_frames(link, when, {})
    live, _, _ = dsn.render_frames(link, when, {}, realtime_since=when.timestamp())

    def track_marks(frame):
        px = frame.load()
        return {x for x in range(dsn.TRACK0, dsn.TRACK1 + 1)
                if px[x, dsn.TRACK_Y] == dsn.PULSE}
...
def test_the_two_modes_do_not_read_the_same():
    ...
    def bottom_row(frame):
        px = frame.load()
        return {(x, y) for x in range(dsn.W) for y in range(dsn.H - 5, dsn.H)
                if px[x, y] != (0, 0, 0)}
...
def test_no_glyph_is_a_solid_block():
    ...
    for ch, glyph in dsn.FONT.items():
        width = len(glyph[0])
...
def test_no_two_glyphs_are_confusable():
    ...
    for ch, glyph in dsn.FONT.items():
        ...
        seen[glyph] = ch
    items = [(c, g) for c, g in dsn.FONT.items() if c != " "]
...
def test_the_font_is_proportional_and_measured_correctly():
    ...
    assert dsn.glyph_width("M") == 5 and dsn.glyph_width("W") == 5
    assert dsn.glyph_width("I") == 3 and dsn.glyph_width("1") == 3
    assert dsn.glyph_width("A") == 4
    assert all(len(g) == 5 for g in dsn.FONT.values()), "all glyphs are 5 tall"
    for ch, glyph in dsn.FONT.items():
...
def test_measured_width_matches_what_is_actually_drawn():
    ...
    for text in ("MW", "III", "VOYAGER 2", "18:43", "246KBPS", "M1I W", ""):
        img = Image.new("RGB", (dsn.W, dsn.H), (0, 0, 0))
        dsn._text(img.load(), 0, 0, text, (255, 255, 255))
        px = img.load()
        lit = [x for x in range(dsn.W) for y in range(5) if px[x, y] != (0, 0, 0)]
        measured = dsn.text_width(text)
...
def test_every_simultaneous_receive_record_is_preserved_without_a_false_total():
    ...
    link = dsn.parse_feed(RICH_FEED)[0]
...
def test_the_rare_flags_are_read():
    ...
    plain = dsn.parse_feed(FEED)[0]
    assert not any((plain.mspa, plain.arrayed, plain.ddor))


def test_power_is_said_in_units_a_person_can_feel():
    assert dsn.power_words(-140.0) == "10 attowatts"      # 1e-17 W, Juno
    assert dsn.power_words(-130.0) == "100 attowatts"
    assert dsn.power_words(-120.0) == "one femtowatt"     # exactly 1e-15 W
    assert dsn.power_words(-110.0) == "10 femtowatts"
    assert dsn.power_words(-155.0) == "under one attowatt"   # Voyager
    assert dsn.power_words(None) == ""
    assert dsn.power_words(0.0) == ""
...
def test_units_agree_with_their_numbers():
    ...
    assert dsn.rate_words(1_188_000) == "about 1 megabit per second"
    assert dsn.rate_words(2_000_000) == "about 2 megabits per second"
    assert dsn.rate_words(1_000) == "about 1 kilobit per second"
    assert dsn.light_words(0.43) == "less than a second"   # Chandra
    assert dsn.light_words(1.0) == "1 second"
...
def test_distance_is_not_given_in_light_years():
    ...
    voyager = dsn.distance_words(2.1e10)
    ...
    assert dsn.distance_words(9.41e8).startswith("941 million kilometres away")
    assert dsn.distance_words(111_000) == "111 thousand kilometres away"
    assert dsn.distance_words(None) == "" and dsn.distance_words(0) == ""
    # the AU comparison is pointless below about one, and is left off
    assert "times the Earth" not in dsn.distance_words(1.5e6)
...
def test_the_light_year_is_used_as_a_comparison_not_a_unit():
    ...
    assert dsn.lightyear_words(2.1e10).startswith("A single light year is 451")
    assert "nearest star" in dsn.lightyear_words(2.1e10)
    assert dsn.lightyear_words(9.41e8) == "", "Juno is not deep space"
    assert dsn.lightyear_words(None) == ""
...
def test_the_distance_sentence_reads_as_english():
    ...
    link = _voice_link(range_km=2.1e10)
    text = dsn.spoken(link, {"vgr2": "Voyager 2"}, {})
...
def test_an_unknown_craft_still_narrates():
    ...
    text = dsn.spoken(link, {"zzzz": "Some New Probe"}, {})
...
def _globe_pixels(frame):
    ...
    px = frame.load()
    return tuple(px[x, y]
                 for x in range(dsn.GLOBE_CX - dsn.GLOBE_R, dsn.GLOBE_CX + dsn.GLOBE_R + 1)
                 for y in range(dsn.GLOBE_CY - dsn.GLOBE_R, dsn.GLOBE_CY + dsn.GLOBE_R + 1))
...
def test_the_shadow_turns_WITH_the_planet():
    ...
    for hour in (2, 8, 14, 20):
        ...
        frames, _, _ = dsn.render_frames(link, when, {}, site_lons=sites)
...
def test_the_browsing_spin_does_not_depend_on_the_spacecraft():
    ...
    def spin_of(link):
        frames, _, _ = dsn.render_frames(link, when, {})
        ...
        return sum(1 for x in range(dsn.GLOBE_CX - dsn.GLOBE_R,
                                    dsn.GLOBE_CX + dsn.GLOBE_R)
                   for y in range(dsn.GLOBE_CY - dsn.GLOBE_R,
                                  dsn.GLOBE_CY + dsn.GLOBE_R + 1)
                   if px0[x, y] != px1[x, y])
...
def test_the_globe_is_centred_on_the_complex_that_is_listening():
    ...
    def lit_fraction(complex_name, hour):
        ...
        when = datetime(2026, 8, 6, hour, tzinfo=timezone.utc)
        frame = dsn.render_frames(link, when, {}, site_lons=sites)[0][0]
        px = frame.load()
        disc = [(x, y)
                for x in range(dsn.GLOBE_CX - dsn.GLOBE_R, dsn.GLOBE_CX + dsn.GLOBE_R + 1)
                for y in range(dsn.GLOBE_CY - dsn.GLOBE_R, dsn.GLOBE_CY + dsn.GLOBE_R + 1)
...
def test_the_led_blinks_on_events_not_on_every_redraw():
    ...
    assert dsn._scene_payload("x.anim").led_notification_color is None
    assert dsn._scene_payload("x.anim", dsn.LED_ARRIVAL).led_notification_color \
        == dsn.LED_ARRIVAL
    for colour in (dsn.LED_ARRIVAL, dsn.LED_LOCKED, dsn.LED_RELEASED):
...
def test_an_arrival_fires_once_and_then_the_watch_is_over():
    ...
    assert dsn.arrival_due(state, link, lock) is False
    assert dsn.arrival_due(state, link, lock + light * 0.5) is False
    assert dsn.arrival_due(state, link, lock + light + 1) is True
    assert dsn.arrival_due(state, link, lock + light * 2) is False
...
def test_browsing_never_reports_an_arrival():
    ...
    for t in (0.0, 1e9, 2e9):
        assert dsn.arrival_due(state, link, t) is False
...
def test_no_sprite_is_a_solid_blob():
    ...
    cells = dsn.CRAFT_W * dsn.CRAFT_H
    for name, sprite in dsn.SPRITES.items():
        assert len(sprite) == dsn.CRAFT_H, f"{name}: sprites are 6 tall"
        assert {len(r) for r in sprite} == {dsn.CRAFT_W}, f"{name}: ragged"
        ...
        for ch in {c for row in sprite for c in row} - {"."}:
            assert ch in dsn.INK, f"{name}: no colour for {ch!r}"
...
def test_the_sprite_box_stays_inside_the_panel():
    ...
    assert dsn.CRAFT_X + dsn.CRAFT_W == dsn.W
    assert dsn.CRAFT_Y + dsn.CRAFT_H == dsn.H - 5   # clear of the bottom label
    assert dsn.CRAFT_Y == 5                          # clear of the top label
...
def test_craft_map_to_the_shape_they_actually_are():
    ...
    assert dsn.craft_sprite("VGR2") == dsn.SPRITES["voyager"]
    assert dsn.craft_sprite("VGR1") == dsn.craft_sprite("VGR2")  # identical
    assert dsn.craft_sprite("JNO") == dsn.SPRITES["juno"]
    assert dsn.craft_sprite("LUCY") == dsn.SPRITES["lucy"]
    assert dsn.craft_sprite("SPP") == dsn.SPRITES["parker"]
    assert dsn.craft_sprite("JWST") == dsn.SPRITES["jwst"]
    assert dsn.craft_sprite("MRO") == dsn.SPRITES["marsorbiter"]
    # the two on a surface are not orbiters and must not look like one
    assert dsn.craft_sprite("M20") == dsn.SPRITES["rover"]
    assert dsn.craft_sprite("MSL") == dsn.SPRITES["rover"]
    assert dsn.craft_sprite("MER1") == dsn.SPRITES["solarrover"]
    assert dsn.craft_sprite("M20") != dsn.craft_sprite("MRO")
    # an Artemis upper stage is not a spacecraft and should not pretend to be
    assert dsn.craft_sprite("ICPS") == dsn.SPRITES["stage"]
    # anything unmapped still draws a plausible satellite
    assert dsn.craft_sprite("ZZZZ") == dsn.SPRITES[dsn.DEFAULT_SHAPE]
    assert dsn.craft_sprite("") == dsn.SPRITES[dsn.DEFAULT_SHAPE]
    for code in dsn.CRAFT_SHAPES:
        assert dsn.CRAFT_SHAPES[code] in dsn.SPRITES, code


def test_every_sprite_belongs_to_someone():
    """A portrait nobody is mapped to is a portrait of nothing."""
    used = set(dsn.CRAFT_SHAPES.values()) | {dsn.DEFAULT_SHAPE}
    assert set(dsn.SPRITES) == used, f"orphans: {set(dsn.SPRITES) - used}"
...
def test_the_fleet_is_described_as_well_as_drawn():
    ...
    unknown = sorted(c for c in dsn.CRAFT_SHAPES if c not in dsn.MISSIONS)
...
def test_the_sprite_reaches_the_panel():
    ...
    when = datetime(2026, 8, 6, 12, tzinfo=timezone.utc)
    juno = dsn.render_frames(_voice_link(craft="JNO", range_km=8.2e8), when, {})[0][0]
    vgr = dsn.render_frames(_voice_link(craft="VGR2", range_km=2.1e10), when, {})[0][0]

    def craft_area(frame):
        px = frame.load()
        return tuple(px[x, y] for x in range(62, dsn.W)
                     for y in range(dsn.TRACK_Y - 2, dsn.TRACK_Y + 3))
...
def _first_moves(frames, row, colour):
    ...
    seen = [min(xs) for xs in
            ([x for x in range(dsn.TRACK0, dsn.TRACK1 + 1)
...
def test_both_halves_of_the_conversation_are_drawn():
    ...
    frames, _, _ = dsn.render_frames(two_way, when, {})

    def row_has(colour, row):
        return any(f.load()[x, row] == colour
                   for f in frames for x in range(dsn.TRACK0, dsn.TRACK1 + 1))

    assert row_has(dsn.UPLINK, dsn.UP_Y), "Earth's half is missing"
    assert row_has(dsn.PULSE, dsn.DOWN_Y), "the spacecraft's half is missing"
    assert dsn.UP_Y != dsn.DOWN_Y, "the two directions must not share a row"
    # and each must travel the right way
    assert _first_moves(frames, dsn.UP_Y, dsn.UPLINK) > 0, "uplink must go out"
    assert _first_moves(frames, dsn.DOWN_Y, dsn.PULSE) < 0, "downlink comes home"
...
def test_the_uplink_outweighs_what_comes_back():
    ...
    assert widest(dsn.UP_Y, dsn.UPLINK) > widest(dsn.DOWN_Y, dsn.PULSE)
    assert sum(dsn.UPLINK) > sum(dsn.PULSE) * 0.8, "the uplink must read brighter"
    # both stay above the panel's gamma floor against their own tether
    for ink, tether in ((dsn.UPLINK, dsn.UP_TETHER), (dsn.PULSE, dsn.TETHER)):
...
def test_a_one_way_pass_still_draws_its_single_direction():
    ...
    frames, _, _ = dsn.render_frames(up_only, when, {})
    assert _first_moves(frames, dsn.UP_Y, dsn.UPLINK) > 0
    # the quiet direction still shows a link, it just has nothing on it
    assert all(f.load()[dsn.TRACK0, dsn.DOWN_Y] != (0, 0, 0) for f in frames)
...
def test_the_tether_joins_earth_to_the_craft_at_all_times():
    ...
    it."""
    when = datetime(2026, 8, 6, 12, tzinfo=timezone.utc)
    frames, _, _ = dsn.render_frames(_voice_link(range_km=2.95e8), when, {})
    for f in frames:
        px = f.load()
        unlit = [x for x in range(dsn.TRACK0, dsn.TRACK1 + 1)
                 if px[x, dsn.TRACK_Y] == (0, 0, 0)]
    ...
    assert sum(dsn.TETHER) * 3 < sum(dsn.PULSE)
...
def test_the_pulse_length_carries_the_data_rate():
    ...
    assert dsn.pulse_span(2e6) == 3
    assert dsn.pulse_span(18090) == 2
    assert dsn.pulse_span(160) == 1
    assert dsn.pulse_span(0) == 1
    ...
    def widest(bps):
        ...
        for f in frames:
            px, run = f.load(), 0
            for x in range(dsn.TRACK0, dsn.TRACK1 + 1):
                run = run + 1 if px[x, dsn.TRACK_Y] == dsn.PULSE else 0
                runs.append(run)
        return max(runs)
...
def test_the_countdown_says_how_long_until_it_lands():
    ...
    assert dsn.countdown_label(light, lock, lock) == "19:50"
    assert dsn.countdown_label(light, lock, lock + 3600) == "18:50"
    assert dsn.countdown_label(960.0, lock, lock) == "16:00"        # Mars
    assert dsn.countdown_label(960.0, lock, lock + 900) == "1:00"
    assert dsn.countdown_label(960.0, lock, lock + 960) == "0:00"
    assert dsn.countdown_label(None, lock, lock) == "?"
    for label in (dsn.countdown_label(light, lock, lock),
                  dsn.countdown_label(960.0, lock, lock + 930)):
        assert len(label) <= 5, label
        for ch in label:
            assert ch in dsn.FONT, f"{ch!r} would be silently skipped"
...
def test_the_countdown_really_does_finish():
    ...
    assert not dsn.arrived(light, lock, lock)
    assert not dsn.arrived(light, lock, lock + light - 1)
    assert dsn.arrived(light, lock, lock + light)
    assert not dsn.arrived(None, lock, lock + 1e9)


def test_progress_stops_at_arrival_instead_of_wrapping():
    """The watch ends when the signal lands; it must not silently restart."""
    light, lock = 1000.0, 1_780_000_000.0
    assert dsn.realtime_progress(light, lock + 500, lock) == pytest.approx(0.5)
    assert dsn.realtime_progress(light, lock + 1500, lock) == 1.0
...
def test_the_arrival_ends_the_watch_and_hands_back_the_rotation():
    ...
    assert dsn.arrival_due(state, link, lock + 60) is False
    assert state.focus == link.key, "released before it landed"

    assert dsn.arrival_due(state, link, lock + link.light_s + 1) is True
    ...
    assert dsn.arrival_due(state, link, lock + 1e6) is False   # only once
...
def test_every_spacecraft_the_network_publishes_has_a_description():
    ...
    real = {c for c in published if c.upper() not in dsn.NOT_SPACECRAFT}
    missing = sorted(real - set(dsn.MISSIONS))
...
def test_a_line_is_not_baked_before_the_data_it_describes_arrives():
    ...
    assert not dsn.narration_ready(state, link), "baked before the name table"
    state.names = {"vgr2": "Voyager 2"}
    assert not dsn.narration_ready(state, link), "baked before Horizons answered"

    link.range_km = 2.1e10
    assert dsn.narration_ready(state, link)

    # a craft whose id never resolves must not be blocked forever
    assert dsn.narration_ready(state, _voice_link(range_km=None, naif=None))

    # and the gated line really is the fuller one
    thin = dsn.spoken(_voice_link(range_km=None), {}, {})
    full = dsn.spoken(_voice_link(range_km=2.1e10), {"vgr2": "Voyager 2"}, {})
...
def test_bumping_the_cache_generation_retires_the_old_one():
    ...
    assert not dsn.VOICE_FILES.match(old), "old generation still looks current"
    assert dsn.VOICE_FILES.match(dsn.speech_name("x", "af_nova"))
...
def test_the_name_map_keeps_trying(monkeypatch):
    ...
    monkeypatch.setattr(dsn, "fetch_names", flaky)
    sleeps = []
    ...
    state = dsn.State()
    with pytest.raises(asyncio.CancelledError):
        asyncio.run(dsn.poll_names(state))
...
def test_the_antenna_leans_the_way_the_real_dish_leans():
    ...
    low = dsn.dish_tilt(12.0)
    mid = dsn.dish_tilt(41.0)
    high = dsn.dish_tilt(85.0)
    ...
    assert dsn.dish_tilt(90.0) == high
    assert dsn.dish_tilt(0.0) == low                # and a missing angle
    assert dsn.dish_tilt(120.0) == high             # never falls off the end
...
def test_the_antenna_icon_cannot_silently_steal_the_name_box():
    ...
    assert dsn.DISH_ICON_W == 6
    for ceiling, rows in dsn.DISH_TILTS:
        assert all(len(r) == dsn.DISH_ICON_W for r in rows), ceiling
...
def test_the_antenna_is_not_drawn_in_the_digits_colour():
    ...
    assert dsn.ANTENNA != dsn.DISH_NO
    when = datetime(2026, 8, 6, 12, tzinfo=timezone.utc)
    frames, _, _ = dsn.render_frames(_voice_link(elevation=12.0), when, {})
    px = frames[0].load()
    tx = dsn.GLOBE_CX + dsn.GLOBE_R + 3
    drawn = {px[tx + dx, dy]
             for dy in range(6) for dx in range(dsn.DISH_ICON_W)}
    assert dsn.ANTENNA in drawn, "the antenna icon never reached the panel"
...
def test_the_antenna_icon_leaves_the_tether_alone():
    ...
    land on the uplink."""
    when = datetime(2026, 8, 6, 12, tzinfo=timezone.utc)
    frames, _, _ = dsn.render_frames(_voice_link(elevation=85.0), when, {})
    px = frames[0].load()
    tx = dsn.GLOBE_CX + dsn.GLOBE_R + 3
    for dx in range(dsn.DISH_ICON_W):
        assert px[tx + dx, dsn.UP_Y] != dsn.ANTENNA, "icon spilled onto y6"
...
def test_a_glint_lands_on_the_craft_and_not_on_empty_space():
    ...
    catching an array."""
    for shape, path in dsn.CRAFT_GLINT.items():
        sprite = dsn.SPRITES[shape]
        for row, col in path:
            assert 0 <= row < dsn.CRAFT_H and 0 <= col < dsn.CRAFT_W, shape
...
def test_only_craft_that_really_spin_are_animated():
    ...
    for still in ("jwst", "parker", "lucy", "clipper", "rover",
        ...
        assert still not in dsn.CRAFT_GLINT, f"{still} does not spin"
    assert "juno" in dsn.CRAFT_GLINT       # spin-stabilised at 2 rpm
    assert "spinner" in dsn.CRAFT_GLINT    # ACE, Wind, IMAP, Ulysses
...
def test_the_glint_actually_moves_across_the_loop():
    ...
    brighter pixel."""
    when = datetime(2026, 8, 6, 12, tzinfo=timezone.utc)
    frames, _, _ = dsn.render_frames(_voice_link(craft="JNO"), when, {})
    spots = set()
    for f in frames:
        px = f.load()
        for y in range(dsn.CRAFT_Y, dsn.CRAFT_Y + dsn.CRAFT_H):
            for x in range(dsn.CRAFT_X, dsn.W):
                if px[x, y] == dsn.HOT:
                    spots.add((x, y))
    # distinct positions: the path repeats each tip so the sweep runs at
    # Juno's real 2 rpm rather than four times it
    assert len(spots) == len(set(dsn.CRAFT_GLINT["juno"])), \
...
def test_the_sun_is_where_the_sun_actually_is():
    ...
    lat, lon = dsn.subsolar(solstice)
    assert 23.0 < lat < 23.6, f"June solstice declination was {lat}"
    lat, _ = dsn.subsolar(datetime(2026, 12, 21, 12, tzinfo=timezone.utc))
    assert -23.6 < lat < -23.0
    for month in (3, 9):
        lat, _ = dsn.subsolar(datetime(2026, month, 21, 12, tzinfo=timezone.utc))
        assert abs(lat) < 1.5, f"equinox declination was {lat}"
    # the equation of time: noon UTC is NOT the prime meridian's true noon
    offs = [dsn.subsolar(datetime(2026, m, 5, 12, tzinfo=timezone.utc))[1]
            for m in range(1, 13)]
...
def test_the_band_is_finally_said_out_loud():
    ...
    assert "Ka band" in dsn.band_words("Ka")
    assert "S band" in dsn.band_words("S")
    assert "X band" in dsn.band_words("X")
    assert dsn.band_words("") == ""
    text = dsn.spoken(_voice_link(band="Ka"), {"vgr2": "Voyager 2"})
...
def test_the_band_colours_the_downlink():
    ...
    seen = {}
    for band in ("S", "X", "Ka"):
        frames, _, _ = dsn.render_frames(_voice_link(band=band), when, {})
        px = frames[0].load()
        lit = {px[x, dsn.DOWN_Y] for x in range(dsn.TRACK0, dsn.TRACK1 + 1)}
        seen[band] = lit
    assert seen["S"] != seen["X"] != seen["Ka"], "the bands look identical"
    for band, colour in dsn.BAND_PULSE.items():
        r, g, b = colour
...
def test_a_seventy_metre_dish_looks_like_one():
    ...
    small = dsn.dish_tilt(41.0, big=False)
    big = dsn.dish_tilt(41.0, big=True)
    assert small != big
    ...
    for rows in (small, big):
        assert len(rows) == 6 and {len(r) for r in rows} == {dsn.DISH_ICON_W}


def test_arraying_and_mspa_are_drawn_not_just_narrated():
    """A four-dish array used to look exactly like a single dish."""
    when = datetime(2026, 8, 6, 12, tzinfo=timezone.utc)
    plain, _, _ = dsn.render_frames(_voice_link(), when, {})
    for flag, colour in (("arrayed", dsn.ANTENNA), ("mspa", dsn.NAME),
                         ("ddor", dsn.DDOR_MARK)):
        marked, _, _ = dsn.render_frames(_voice_link(**{flag: True}), when, {})
        col = {marked[0].load()[dsn.MODE_X, y] for y in range(5)}
        assert colour in col, f"{flag} left no mark"
        assert col != {plain[0].load()[dsn.MODE_X, y] for y in range(5)}
...
def test_round_trip_time_is_the_number_an_operator_lives_by():
    ...
    text = dsn.spoken(_voice_link(range_km=2.1e10), {"vgr2": "Voyager 2"})
    ...
    assert "immediate round trip" not in dsn.spoken(
...
def test_every_band_the_feed_publishes_is_handled():
    ...
    for band in ("S", "X", "K", "Ka"):
        assert dsn.BAND_PULSE.get(dsn.band_key(band)), f"{band} has no colour"
        assert dsn.band_words(band), f"{band} is narrated in silence"
    # and the shapes the string arrives in
    assert dsn.band_key(" ka ") == dsn.band_key("Ka") == dsn.band_key("KA")
    assert dsn.band_key("Ka-band") == "KA"
    assert dsn.band_key("") == ""
    assert dsn.band_words("") == ""          # unknown stays quiet, not wrong
...
def _snap(*pairs, **flags):
    ...
    return [dsn.Link(complex_name="Canberra", dish=d, craft=c, elevation=40.0,
...
def test_history_records_transitions_not_polls():
    ...
    events = dsn.link_events(before, after, 1000.0)
    ...
    assert dsn.link_events(after, after, 1000.0) == []
...
def test_history_survives_a_restart(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(dsn, "HISTORY_PATH", tmp_path / "h.jsonl")
    dsn.append_history(dsn.link_events([], _snap(("DSS43", "VGR2")), 100.0))
    dsn.append_history(dsn.link_events([], _snap(("DSS14", "VGR2")), 200.0))

    state = dsn.State()
    dsn.load_history(state)
...
def test_a_half_written_line_is_skipped_not_fatal(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(dsn, "HISTORY_PATH", path)
...
def test_history_stays_bounded(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(dsn, "HISTORY_PATH", tmp_path / "h.jsonl")
    monkeypatch.setattr(dsn, "HISTORY_MAX_BYTES", 2000)
    monkeypatch.setattr(dsn, "HISTORY_MAX_LINES", 10)
    for i in range(400):
        dsn.append_history([{"t": float(i), "event": "appear",
    ...
    assert path.stat().st_size < dsn.HISTORY_MAX_BYTES * 2
...
def test_the_history_cap_leaves_headroom():
    ...
    after_trim = dsn.HISTORY_MAX_LINES * 110      # generous bytes per line
    assert after_trim < dsn.HISTORY_MAX_BYTES * 0.75, (
...
def test_a_missing_history_file_is_a_cold_start_not_a_crash(tmp_path, monkeypatch):
    monkeypatch.setattr(dsn, "HISTORY_PATH", tmp_path / "nope.jsonl")
    state = dsn.State()
    dsn.load_history(state)
    assert state.seen == {}
