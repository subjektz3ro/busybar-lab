...
def test_the_bomb_is_well_under_the_byte_budget():
    ...
    assert len(BILLION_LAUGHS) < dsn_source.FEED_XML_MAX_BYTES / 1000


def test_entity_expansion_is_refused_by_parse_feed():
    with pytest.raises(EntitiesForbidden):
        dsn_source.parse_feed(BILLION_LAUGHS)


def test_entity_expansion_is_refused_by_parse_config():
    with pytest.raises(EntitiesForbidden):
        dsn_source.parse_config(BILLION_LAUGHS)
...
def test_the_timestamp_reader_does_not_expand_either():
    ...
    assert dsn_source.feed_timestamp_ms(BILLION_LAUGHS) is None
...
def test_an_external_entity_is_refused():
    ...
    assert dsn_source.feed_timestamp_ms(EXTERNAL_ENTITY) is None
    with pytest.raises(EntitiesForbidden):
        dsn_source.parse_feed(EXTERNAL_ENTITY)
...
def test_ordinary_feeds_still_parse():
    ...
    assert dsn_source.feed_timestamp_ms(good) == 1780000000000
    assert dsn_source.parse_feed(good) == []
