...
def pinned_sky(monkeypatch):
    ...
    """
    monkeypatch.setattr(sky_settings, "OBSERVER", Observer(latitude=0.0, longitude=0.0))
...
@pytest.mark.parametrize("name", sorted(sky_config.STATUS_INKS) + ["scrubbed"])
def test_every_offered_ink_clears_the_measured_background_extremes(name):
    ...
    ink = (sky_render_status.STATUS_INK_SCRUBBED if name == "scrubbed"
           else sky_config.STATUS_INKS[name])
...
@pytest.mark.parametrize("name", sorted(sky_config.STATUS_INKS))
def test_the_scrub_tell_is_distinguishable_from_every_live_ink(name):
    ...
    per_channel = [abs(a - b) for a, b in zip(
        sky_config.STATUS_INKS[name], sky_render_status.STATUS_INK_SCRUBBED)]
...
def test_the_clock_colour_is_identical_across_every_frame_of_a_loop(pinned_sky):
    ...
    wx = sky_weather.WeatherState(cloud_frac=0.2, temp_c=20.0, thunder=True)
    frames = sky_render_scene.render_loop_frames(now, wx, seed=0, scene="house")
    inks = set()
    for frame in frames:
        px = frame.load()
        inks |= {px[x, y] for y in range(0, 7) for x in range(0, 20)}
    assert sky_settings.CLOCK_INK in inks
    assert len(frames) > 1
...
def _glyph_cells(text: str) -> set[tuple[int, int]]:
    ...
    cells, cx = set(), max(1, (sky_limits.STATUS_CARD_W - text_w) // 2)
    for ch in text:
        glyph = sky_render_art.DIGITS_3X5[ch]
        for gy, row in enumerate(glyph):
            for gx, bit in enumerate(row):
                if bit == "1" and 0 <= cx + gx < sky_limits.W:
...
def _on_panel(points):
    return {(x, y) for x, y in points if 0 <= x < sky_limits.W and 0 <= y < sky_limits.H}
...
def test_every_neighbour_of_every_stroke_clears_the_floor(
    ...
    wx = sky_weather.WeatherState(cloud_frac=cloud, temp_c=20.0)
    cells = _glyph_cells(sky_render_status.clock_str(now.astimezone(sky_settings.TZ)))
    for scene in sky_settings.ENABLED_SCENES:
        px = sky_render_scene.render_scene(now, wx, seed=0, scene=scene).load()
        bad = [(n, px[n]) for n in _on_panel(_halo(cells))
               if not clears(sky_settings.CLOCK_INK, px[n])]
...
def test_the_scene_is_untouched_around_the_text(pinned_sky):
    ...
    now = datetime(2026, 6, 15, 12, 0, tzinfo=timezone.utc)
    px = sky_render_scene.render_scene(now, sky_weather.WeatherState(temp_c=20.0), seed=0,
                         scene="house").load()
    halo = _on_panel(_halo(_glyph_cells(sky_render_status.clock_str(now.astimezone(sky_settings.TZ)))))
...
def test_the_strokes_themselves_are_still_drawn(pinned_sky):
    ...
    for x, y in cells:
        assert px[x, y] == sky_settings.CLOCK_INK, f"stroke missing at ({x},{y})"
...
def test_the_scrubbed_clock_is_drawn_in_amber(pinned_sky):
    ...
    cells = _glyph_cells(sky_render_status.clock_str(now.astimezone(sky_settings.TZ)))
    for x, y in cells:
        assert px[x, y] == sky_render_status.STATUS_INK_SCRUBBED
...
def test_no_star_spawns_inside_the_status_corner():
    ...
    for x, y, _mag in sky_render_art.STARS:
        assert x >= sky_limits.STATUS_CARD_W or y > 6, (x, y)
